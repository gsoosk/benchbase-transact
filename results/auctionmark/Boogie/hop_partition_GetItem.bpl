...
procedure verify_hop_partitions_GetItem(item_id: int, seller_id: int)
{
  var s5_#tmp4 : Row (Table (ITEM));
  var s5_item_id : int;
  var s5_seller_id : int;
  var s5_item : Row (Table (ITEM));
  var s5_#tmp5 : unit;
  var s5_seller#u_r_id : int;
  var s5_#tmp7 : Row (Table (REGION));
  var s5_region : Row (Table (REGION));
  var s5_#tmp8 : unit;

  // Hop partition verification for function 'GetItem'
  s5_block5:
    s5_#tmp4 := construct_Row_ITEM(ITEM_i_id[s5_item_id][s5_seller_id], ITEM_i_u_id[s5_item_id][s5_seller_id], ITEM_i_name[s5_item_id][s5_seller_id], ITEM_i_initial_price[s5_item_id][s5_seller_id], ITEM_i_current_price[s5_item_id][s5_seller_id], ITEM_i_num_bids[s5_item_id][s5_seller_id], ITEM_i_end_date[s5_item_id][s5_seller_id], ITEM_i_start_date[s5_item_id][s5_seller_id], ITEM_i_status[s5_item_id][s5_seller_id], ITEM_i_num_comments[s5_item_id][s5_seller_id], ITEM_i_c_id[s5_item_id][s5_seller_id]);
    s5_item := s5_#tmp4;
    s5_#tmp5 := to_unit(s5_item);
    s5_seller#u_r_id := USERACCT_u_r_id[s5_seller_id];
  // Partition check hop 5 func 'p_user' tables 'ITEM'=>'USERACCT' keys [k0=seller_id] first_span Span { start: 7069, end: 7107, filename: "/Users/farzad/Desktop/Research/benchbase-transact/auctionmark.transact" } current_span Span { start: 7168, end: 7193, filename: "/Users/farzad/Desktop/Research/benchbase-transact/auctionmark.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 19) (hop_id . 5) (table_id . 0) (span ((start . 7168) (end . 7193) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/auctionmark.transact\"))))"} (s5_seller_id == s5_seller_id);
    s5_#tmp7 := construct_Row_REGION(REGION_r_id[s5_seller#u_r_id], REGION_r_name[s5_seller#u_r_id]);
    s5_region := s5_#tmp7;
    s5_#tmp8 := to_unit(s5_region);
    goto s5_epilogue;
  s5_hop_exit:
  s5_epilogue:
}
