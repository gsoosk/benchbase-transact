...
procedure Check_SliceCommut_Hop12_vs_Hop14()
modifies ITEM_i_c_id, ITEM_i_current_price, ITEM_i_end_date, ITEM_i_initial_price, ITEM_i_name, ITEM_i_num_bids, ITEM_i_num_comments, ITEM_i_start_date, ITEM_i_status, USERACCT_u_balance, USERACCT_u_comments, USERACCT_u_id, USERACCT_u_r_id, USERACCT_u_rating, USERACCT_u_sattr0, USERACCT_u_sattr1, USERACCT_u_sattr2, USERACCT_u_sattr3, USERACCT_u_sattr4;
{
  var s0_item_id : int;
  var s0_seller_id : int;
  var s0_name : String;
  var s0_initial_price : real;
  var s0_end_date : int;
  var s0_start_date : int;
  var s0_c_id : int;
  var s1_buyer_id : int;
  ...
  var s0_initial_price_init : real;
  var s0_end_date_init : int;
  var s0_start_date_init : int;
  ...
  // Slice commutativity verification: hop 12 vs hop 14
  ...
  // --- Step 2: Save initial state ---
    ...
    s0_initial_price_init := s0_initial_price;
    s0_end_date_init := s0_end_date;
    s0_start_date_init := s0_start_date;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block16__ab:
      ITEM_i_name := ITEM_i_name[s0_item_id := ITEM_i_name[s0_item_id][s0_seller_id := s0_name]];
      ITEM_i_initial_price := ITEM_i_initial_price[s0_item_id := ITEM_i_initial_price[s0_item_id][s0_seller_id := s0_initial_price]];
      ITEM_i_current_price := ITEM_i_current_price[s0_item_id := ITEM_i_current_price[s0_item_id][s0_seller_id := s0_initial_price]];
      ITEM_i_num_bids := ITEM_i_num_bids[s0_item_id := ITEM_i_num_bids[s0_item_id][s0_seller_id := 0]];
      ITEM_i_end_date := ITEM_i_end_date[s0_item_id := ITEM_i_end_date[s0_item_id][s0_seller_id := s0_end_date]];
      ITEM_i_start_date := ITEM_i_start_date[s0_item_id := ITEM_i_start_date[s0_item_id][s0_seller_id := s0_start_date]];
      ITEM_i_status := ITEM_i_status[s0_item_id := ITEM_i_status[s0_item_id][s0_seller_id := ITEM_STATUS_OPEN]];
      ITEM_i_num_comments := ITEM_i_num_comments[s0_item_id := ITEM_i_num_comments[s0_item_id][s0_seller_id := 0]];
      ITEM_i_c_id := ITEM_i_c_id[s0_item_id := ITEM_i_c_id[s0_item_id][s0_seller_id := s0_c_id]];
      ...
      s0_#tmp35 := s0_seller#u_balance - s0_initial_price;
      s0_seller#u_balance := s0_#tmp35;
  ...
  // Restoring initial state:
    ...
    s0_initial_price := s0_initial_price_init;
    s0_end_date := s0_end_date_init;
    s0_start_date := s0_start_date_init;
