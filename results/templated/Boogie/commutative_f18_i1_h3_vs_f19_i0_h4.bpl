...
modifies TEMPLATED_TABLE_T_FLOAT_VAL, TEMPLATED_TABLE_T_ID, TEMPLATED_TABLE_T_INT_VAL, TEMPLATED_TABLE_T_STRING_VAL;
{
  var s0_key : int;
  var s0_int_val : int;
  var s0_float_val : real;
  var s0_string_val : String;
  var s1_key1 : int;
  var s1_key2 : int;
  var s1_amount : real;
  var s0_active : bool;
  ...
  var s0_key_init : int;
  var s0_int_val_init : int;
  var s0_float_val_init : real;
  var s0_string_val_init : String;
  ...
  // Slice commutativity verification: hop 3 vs hop 4
  ...
  // --- Step 2: Save initial state ---
    ...
    s0_key_init := s0_key;
    s0_int_val_init := s0_int_val;
    s0_float_val_init := s0_float_val;
    s0_string_val_init := s0_string_val;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block3__ab:
      TEMPLATED_TABLE_T_INT_VAL := TEMPLATED_TABLE_T_INT_VAL[s0_key := s0_int_val];
      TEMPLATED_TABLE_T_FLOAT_VAL := TEMPLATED_TABLE_T_FLOAT_VAL[s0_key := s0_float_val];
      TEMPLATED_TABLE_T_STRING_VAL := TEMPLATED_TABLE_T_STRING_VAL[s0_key := s0_string_val];
      s0_active := false;
  ...
  // Restoring initial state:
    ...
    s0_key := s0_key_init;
    s0_int_val := s0_int_val_init;
    s0_float_val := s0_float_val_init;
    s0_string_val := s0_string_val_init;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 18) (instance . 1) (hop_id . 3)) (node_2 (function_id . 19) (instance . 0) (hop_id . 4)))"} ((((((TEMPLATED_TABLE_T_FLOAT_VAL_a_then_b == TEMPLATED_TABLE_T_FLOAT_VAL_b_then_a) && (TEMPLATED_TABLE_T_ID_a_then_b == TEMPLATED_TABLE_T_ID_b_then_a)) && (TEMPLATED_TABLE_T_INT_VAL_a_then_b == TEMPLATED_TABLE_T_INT_VAL_b_then_a)) && (TEMPLATED_TABLE_T_STRING_VAL_a_then_b == TEMPLATED_TABLE_T_STRING_VAL_b_then_a)) && (s1_key2_a_then_b == s1_key2_b_then_a)) && (s1_amount_a_then_b == s1_amount_b_then_a));
}
