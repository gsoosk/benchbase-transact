...
modifies TEMPLATED_TABLE_T_FLOAT_VAL, TEMPLATED_TABLE_T_ID, TEMPLATED_TABLE_T_INT_VAL, TEMPLATED_TABLE_T_STRING_VAL;
{
  ...
  var s0_key_init : int;
  var s1_key_init : int;
  var s1_new_string_init : String;
  var s0_#tmp1 : Row (Table (TEMPLATED_TABLE));
  var s0_r : Row (Table (TEMPLATED_TABLE));
  var s0_#tmp2 : unit;
  var s1_r#T_FLOAT_VAL : real;
  var s1_r#T_ID : int;
  var s1_r#T_INT_VAL : int;
  var s1_r#T_STRING_VAL : String;
  ...
  // --- Step 2: Save initial state ---
    ...
    s0_key_init := s0_key;
    s1_key_init := s1_key;
    s1_new_string_init := s1_new_string;
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block2__ab:
      s1_r#T_FLOAT_VAL := TEMPLATED_TABLE_T_FLOAT_VAL[s1_key];
      s1_r#T_ID := TEMPLATED_TABLE_T_ID[s1_key];
      s1_r#T_INT_VAL := TEMPLATED_TABLE_T_INT_VAL[s1_key];
      s1_r#T_STRING_VAL := s1_new_string;
      TEMPLATED_TABLE_T_FLOAT_VAL := TEMPLATED_TABLE_T_FLOAT_VAL[s1_key := s1_r#T_FLOAT_VAL];
      TEMPLATED_TABLE_T_ID := TEMPLATED_TABLE_T_ID[s1_key := s1_r#T_ID];
      TEMPLATED_TABLE_T_INT_VAL := TEMPLATED_TABLE_T_INT_VAL[s1_key := s1_r#T_INT_VAL];
      TEMPLATED_TABLE_T_STRING_VAL := TEMPLATED_TABLE_T_STRING_VAL[s1_key := s1_r#T_STRING_VAL];
      s1_active := false;
  ...
  // Restoring initial state:
    ...
    s0_key := s0_key_init;
    s1_key := s1_key_init;
    s1_new_string := s1_new_string_init;
    s0_active := true;
    s1_active := true;
  // Executing B then A:
    if (s1_active) {
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 0) (hop_id . 1)) (node_2 (function_id . 17) (instance . 1) (hop_id . 2)))"} ((((TEMPLATED_TABLE_T_FLOAT_VAL_a_then_b == TEMPLATED_TABLE_T_FLOAT_VAL_b_then_a) && (TEMPLATED_TABLE_T_ID_a_then_b == TEMPLATED_TABLE_T_ID_b_then_a)) && (TEMPLATED_TABLE_T_INT_VAL_a_then_b == TEMPLATED_TABLE_T_INT_VAL_b_then_a)) && (TEMPLATED_TABLE_T_STRING_VAL_a_then_b == TEMPLATED_TABLE_T_STRING_VAL_b_then_a));
}
