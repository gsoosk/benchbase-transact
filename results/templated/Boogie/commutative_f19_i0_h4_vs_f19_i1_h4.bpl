...
procedure Check_SliceCommut_Hop4_vs_Hop4()
modifies TEMPLATED_TABLE_T_FLOAT_VAL, TEMPLATED_TABLE_T_ID, TEMPLATED_TABLE_T_INT_VAL, TEMPLATED_TABLE_T_STRING_VAL;
{
  ...
  var s0_key1_init : int;
  var s0_key2_init : int;
  var s0_amount_init : real;
  var s1_key1_init : int;
  var s1_key2_init : int;
  var s1_amount_init : real;
  var s0_r1#T_FLOAT_VAL : real;
  var s0_r1#T_ID : int;
  var s0_r1#T_INT_VAL : int;
  var s0_r1#T_STRING_VAL : String;
  var s0_#tmp4 : real;
  var s1_r1#T_FLOAT_VAL : real;
  var s1_r1#T_ID : int;
  var s1_r1#T_INT_VAL : int;
  var s1_r1#T_STRING_VAL : String;
  var s1_#tmp4 : real;
  ...
  var s0_key2_a_then_b : int;
  var s0_amount_a_then_b : real;
  var s1_key2_a_then_b : int;
  var s1_amount_a_then_b : real;
  ...
  var s0_key2_b_then_a : int;
  var s0_amount_b_then_a : real;
  var s1_key2_b_then_a : int;
  var s1_amount_b_then_a : real;

  // Slice commutativity verification: hop 4 vs hop 4
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block4__ab:
      s0_r1#T_FLOAT_VAL := TEMPLATED_TABLE_T_FLOAT_VAL[s0_key1];
      s0_r1#T_ID := TEMPLATED_TABLE_T_ID[s0_key1];
      s0_r1#T_INT_VAL := TEMPLATED_TABLE_T_INT_VAL[s0_key1];
      s0_r1#T_STRING_VAL := TEMPLATED_TABLE_T_STRING_VAL[s0_key1];
      s0_#tmp4 := s0_r1#T_FLOAT_VAL - s0_amount;
      s0_r1#T_FLOAT_VAL := s0_#tmp4;
      TEMPLATED_TABLE_T_FLOAT_VAL := TEMPLATED_TABLE_T_FLOAT_VAL[s0_key1 := s0_r1#T_FLOAT_VAL];
      TEMPLATED_TABLE_T_ID := TEMPLATED_TABLE_T_ID[s0_key1 := s0_r1#T_ID];
      TEMPLATED_TABLE_T_INT_VAL := TEMPLATED_TABLE_T_INT_VAL[s0_key1 := s0_r1#T_INT_VAL];
      TEMPLATED_TABLE_T_STRING_VAL := TEMPLATED_TABLE_T_STRING_VAL[s0_key1 := s0_r1#T_STRING_VAL];
      goto s0_hop_exit__ab;
    s0_hop_exit__ab:
    }
    if (s1_active) {
    s1_block4__ab:
      s1_r1#T_FLOAT_VAL := TEMPLATED_TABLE_T_FLOAT_VAL[s1_key1];
      s1_r1#T_ID := TEMPLATED_TABLE_T_ID[s1_key1];
      s1_r1#T_INT_VAL := TEMPLATED_TABLE_T_INT_VAL[s1_key1];
      s1_r1#T_STRING_VAL := TEMPLATED_TABLE_T_STRING_VAL[s1_key1];
      s1_#tmp4 := s1_r1#T_FLOAT_VAL - s1_amount;
      s1_r1#T_FLOAT_VAL := s1_#tmp4;
      TEMPLATED_TABLE_T_FLOAT_VAL := TEMPLATED_TABLE_T_FLOAT_VAL[s1_key1 := s1_r1#T_FLOAT_VAL];
      TEMPLATED_TABLE_T_ID := TEMPLATED_TABLE_T_ID[s1_key1 := s1_r1#T_ID];
      TEMPLATED_TABLE_T_INT_VAL := TEMPLATED_TABLE_T_INT_VAL[s1_key1 := s1_r1#T_INT_VAL];
      TEMPLATED_TABLE_T_STRING_VAL := TEMPLATED_TABLE_T_STRING_VAL[s1_key1 := s1_r1#T_STRING_VAL];
  ...
  // Snapshotting final state for a_then_b
    ...
    s0_key2_a_then_b := s0_key2;
    s0_amount_a_then_b := s0_amount;
    s1_key2_a_then_b := s1_key2;
    s1_amount_a_then_b := s1_amount;
  ...
  // Snapshotting final state for b_then_a
    ...
    s0_key2_b_then_a := s0_key2;
    s0_amount_b_then_a := s0_amount;
    s1_key2_b_then_a := s1_key2;
    s1_amount_b_then_a := s1_amount;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 19) (instance . 0) (hop_id . 4)) (node_2 (function_id . 19) (instance . 1) (hop_id . 4)))"} ((((((((TEMPLATED_TABLE_T_FLOAT_VAL_a_then_b == TEMPLATED_TABLE_T_FLOAT_VAL_b_then_a) && (TEMPLATED_TABLE_T_ID_a_then_b == TEMPLATED_TABLE_T_ID_b_then_a)) && (TEMPLATED_TABLE_T_INT_VAL_a_then_b == TEMPLATED_TABLE_T_INT_VAL_b_then_a)) && (TEMPLATED_TABLE_T_STRING_VAL_a_then_b == TEMPLATED_TABLE_T_STRING_VAL_b_then_a)) && (s0_key2_a_then_b == s0_key2_b_then_a)) && (s0_amount_a_then_b == s0_amount_b_then_a)) && (s1_key2_a_then_b == s1_key2_b_then_a)) && (s1_amount_a_then_b == s1_amount_b_then_a));
}
