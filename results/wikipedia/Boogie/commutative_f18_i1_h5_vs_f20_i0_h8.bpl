...
modifies page_page_id, page_page_is_new, page_page_is_redirect, page_page_latest, page_page_len, page_page_namespace, page_page_title, page_page_touched, page_restrictions_pr_level, recentchanges_rc_comment, recentchanges_rc_cur_id, recentchanges_rc_ip, recentchanges_rc_last_oldid, recentchanges_rc_namespace, recentchanges_rc_new_len, recentchanges_rc_old_len, recentchanges_rc_this_oldid, recentchanges_rc_timestamp, recentchanges_rc_title, recentchanges_rc_user, recentchanges_rc_user_text, revision_rev_comment, revision_rev_len, revision_rev_parent_id, revision_rev_text_id, revision_rev_timestamp, revision_rev_user, revision_rev_user_text, text_old_flags, text_old_page, text_old_text;
{
  ...
  var page_restrictions_pr_level_init : [int][String]String;
  ...
  var s0_pr_type_init : String;
  var s0_latest_rev_id_init : int;
  ...
  var s0_#tmp15 : Row (Table (page));
  var s0_p : Row (Table (page));
  var s0_#tmp16 : unit;
  var s0_#tmp17 : Row (Table (page_restrictions));
  var s0_pr : Row (Table (page_restrictions));
  var s0_#tmp18 : unit;
  var s0_r#rev_text_id : int;
  var s0_textId : int;
  ...
  // Slice commutativity verification: hop 5 vs hop 8
  // --- Step 1: Havoc initial state ---
    ...
    havoc page_restrictions_pr_level;
  ...
  // --- Step 2: Save initial state ---
    ...
    page_restrictions_pr_level_init := page_restrictions_pr_level;
    ...
    s0_pr_type_init := s0_pr_type;
    s0_latest_rev_id_init := s0_latest_rev_id;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block7__ab:
      s0_#tmp15 := construct_Row_page(page_page_id[s0_pageId], page_page_namespace[s0_pageId], page_page_title[s0_pageId], page_page_latest[s0_pageId], page_page_touched[s0_pageId], page_page_is_new[s0_pageId], page_page_is_redirect[s0_pageId], page_page_len[s0_pageId]);
      s0_p := s0_#tmp15;
      s0_#tmp16 := to_unit(s0_p);
      s0_#tmp17 := construct_Row_page_restrictions(page_restrictions_pr_page[s0_pageId][s0_pr_type], page_restrictions_pr_type[s0_pageId][s0_pr_type], page_restrictions_pr_level[s0_pageId][s0_pr_type]);
      s0_pr := s0_#tmp17;
      s0_#tmp18 := to_unit(s0_pr);
      s0_r#rev_text_id := revision_rev_text_id[s0_pageId][s0_latest_rev_id];
      s0_textId := s0_r#rev_text_id;
  ...
  // Restoring initial state:
    ...
    page_restrictions_pr_level := page_restrictions_pr_level_init;
    ...
    s0_pr_type := s0_pr_type_init;
    s0_latest_rev_id := s0_latest_rev_id_init;
