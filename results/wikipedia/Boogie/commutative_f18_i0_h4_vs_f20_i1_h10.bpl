...
procedure Check_SliceCommut_Hop4_vs_Hop10()
modifies ipblocks_ipb_address, ipblocks_ipb_expiry, logging_log_action, logging_log_comment, logging_log_namespace, logging_log_page, logging_log_params, logging_log_timestamp, logging_log_title, logging_log_type, logging_log_user_text, user_user_editcount, user_user_id, user_user_name, user_user_touched;
{
  var s0_userId : int;
  var s0_pageId : int;
  var s0_ug_group : String;
  ...
  var s0_active : bool;
  var s1_active : bool;
  var ipblocks_ipb_address_init : [int][int]String;
  var ipblocks_ipb_expiry_init : [int][int]int;
  ...
  var s0_#tmp9 : Row (Table (user));
  var s0_u : Row (Table (user));
  var s0_#tmp10 : unit;
  var s0_#tmp11 : Row (Table (user_groups));
  var s0_ug : Row (Table (user_groups));
  var s0_#tmp12 : unit;
  var s0_#tmp13 : Row (Table (ipblocks));
  var s0_ip : Row (Table (ipblocks));
  var s0_#tmp14 : unit;
  ...
  var s0_pr_type_a_then_b : String;
  var s0_latest_rev_id_a_then_b : int;
  ...
  var s0_pr_type_b_then_a : String;
  var s0_latest_rev_id_b_then_a : int;

  // Slice commutativity verification: hop 4 vs hop 10
  // --- Step 1: Havoc initial state ---
    havoc ipblocks_ipb_address;
    havoc ipblocks_ipb_expiry;
  ...
  // --- Step 2: Save initial state ---
    ipblocks_ipb_address_init := ipblocks_ipb_address;
    ipblocks_ipb_expiry_init := ipblocks_ipb_expiry;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block6__ab:
      s0_#tmp9 := construct_Row_user(user_user_id[s0_userId], user_user_name[s0_userId], user_user_touched[s0_userId], user_user_editcount[s0_userId]);
      s0_u := s0_#tmp9;
      s0_#tmp10 := to_unit(s0_u);
      s0_#tmp11 := construct_Row_user_groups(user_groups_ug_user[s0_userId][s0_ug_group], user_groups_ug_group[s0_userId][s0_ug_group]);
      s0_ug := s0_#tmp11;
      s0_#tmp12 := to_unit(s0_ug);
      s0_#tmp13 := construct_Row_ipblocks(ipblocks_ipb_user[s0_userId][s0_ipb_id], ipblocks_ipb_id[s0_userId][s0_ipb_id], ipblocks_ipb_address[s0_userId][s0_ipb_id], ipblocks_ipb_expiry[s0_userId][s0_ipb_id]);
      s0_ip := s0_#tmp13;
      s0_#tmp14 := to_unit(s0_ip);
  ...
  // Snapshotting final state for a_then_b
    ...
    s0_pr_type_a_then_b := s0_pr_type;
    s0_latest_rev_id_a_then_b := s0_latest_rev_id;
  // Restoring initial state:
    ipblocks_ipb_address := ipblocks_ipb_address_init;
    ipblocks_ipb_expiry := ipblocks_ipb_expiry_init;
