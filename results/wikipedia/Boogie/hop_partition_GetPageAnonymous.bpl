...
procedure verify_hop_partitions_GetPageAnonymous(pageId: int, pr_type: String, latest_rev_id: int, text_id: int)
{
  var s2_#tmp2 : Row (Table (page));
  var s2_pageId : int;
  var s2_p : Row (Table (page));
  var s2_#tmp3 : unit;
  var s2_#tmp4 : Row (Table (page_restrictions));
  var s2_pr_type : String;
  var s2_pr : Row (Table (page_restrictions));
  var s2_#tmp5 : unit;
  var s2_r#rev_text_id : int;
  var s2_latest_rev_id : int;
  var s2_textId : int;
  var s3_t#old_text : String;
  var s3_textId : int;
  var s3_content : String;
  var s3_#tmp8 : unit;

  // Hop partition verification for function 'GetPageAnonymous'
  s2_block4:
    s2_#tmp2 := construct_Row_page(page_page_id[s2_pageId], page_page_namespace[s2_pageId], page_page_title[s2_pageId], page_page_latest[s2_pageId], page_page_touched[s2_pageId], page_page_is_new[s2_pageId], page_page_is_redirect[s2_pageId], page_page_len[s2_pageId]);
    s2_p := s2_#tmp2;
    s2_#tmp3 := to_unit(s2_p);
    s2_#tmp4 := construct_Row_page_restrictions(page_restrictions_pr_page[s2_pageId][s2_pr_type], page_restrictions_pr_type[s2_pageId][s2_pr_type], page_restrictions_pr_level[s2_pageId][s2_pr_type]);
  // Partition check hop 2 func 'f' tables 'page'=>'page_restrictions' keys [k0=pageId] first_span Span { start: 5995, end: 6016, filename: "/Users/farzad/Desktop/Research/benchbase-transact/wikipedia.transact" } current_span Span { start: 6079, end: 6131, filename: "/Users/farzad/Desktop/Research/benchbase-transact/wikipedia.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 17) (hop_id . 2) (table_id . 3) (span ((start . 6079) (end . 6131) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/wikipedia.transact\"))))"} (s2_pageId == s2_pageId);
    s2_pr := s2_#tmp4;
    s2_#tmp5 := to_unit(s2_pr);
    s2_r#rev_text_id := revision_rev_text_id[s2_pageId][s2_latest_rev_id];
  // Partition check hop 2 func 'f' tables 'page'=>'revision' keys [k0=pageId] first_span Span { start: 5995, end: 6016, filename: "/Users/farzad/Desktop/Research/benchbase-transact/wikipedia.transact" } current_span Span { start: 6185, end: 6234, filename: "/Users/farzad/Desktop/Research/benchbase-transact/wikipedia.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 17) (hop_id . 2) (table_id . 5) (span ((start . 6185) (end . 6234) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/wikipedia.transact\"))))"} (s2_pageId == s2_pageId);
    s2_textId := s2_r#rev_text_id;
    goto s2_hop_exit;
  s2_hop_exit:
  s3_block5:
    s3_t#old_text := text_old_text[s3_textId];
    s3_content := s3_t#old_text;
    s3_#tmp8 := to_unit(s3_content);
    goto s3_epilogue;
  s3_hop_exit:
  s2_epilogue:
  s3_epilogue:
