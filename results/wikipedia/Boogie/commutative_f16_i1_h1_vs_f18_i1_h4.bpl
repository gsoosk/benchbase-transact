...
procedure Check_SliceCommut_Hop1_vs_Hop4()
modifies ipblocks_ipb_address, ipblocks_ipb_expiry, user_user_editcount, user_user_id, user_user_name, user_user_touched, watchlist_wl_notificationtimestamp;
{
  var s0_userId : int;
  ...
  var s1_userId : int;
  var s1_pageId : int;
  var s1_ug_group : String;
  var s1_pr_type : String;
  var s1_latest_rev_id : int;
  var s1_ipb_id : int;
  var s0_active : bool;
  var s1_active : bool;
  ...
  var s1_ug_group_init : String;
  var s1_pr_type_init : String;
  var s1_latest_rev_id_init : int;
  var s1_ipb_id_init : int;
  var s0_#tmp0 : bool;
  var s0_u#user_editcount : int;
  var s0_u#user_id : int;
  var s0_u#user_name : String;
  var s1_#tmp9 : Row (Table (user));
  var s1_u : Row (Table (user));
  var s1_#tmp10 : unit;
  var s1_#tmp11 : Row (Table (user_groups));
  var s1_ug : Row (Table (user_groups));
  var s1_#tmp12 : unit;
  var s1_#tmp13 : Row (Table (ipblocks));
  var s1_ip : Row (Table (ipblocks));
  var s1_#tmp14 : unit;
  ...
  var s1_pr_type_a_then_b : String;
  var s1_latest_rev_id_a_then_b : int;
  ...
  var s1_pr_type_b_then_a : String;
  var s1_latest_rev_id_b_then_a : int;

  // Slice commutativity verification: hop 1 vs hop 4
  ...
  // --- Step 2: Save initial state ---
    ...
    s1_ug_group_init := s1_ug_group;
    s1_pr_type_init := s1_pr_type;
    s1_latest_rev_id_init := s1_latest_rev_id;
    s1_ipb_id_init := s1_ipb_id;
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block6__ab:
      s1_#tmp9 := construct_Row_user(user_user_id[s1_userId], user_user_name[s1_userId], user_user_touched[s1_userId], user_user_editcount[s1_userId]);
      s1_u := s1_#tmp9;
      s1_#tmp10 := to_unit(s1_u);
      s1_#tmp11 := construct_Row_user_groups(user_groups_ug_user[s1_userId][s1_ug_group], user_groups_ug_group[s1_userId][s1_ug_group]);
      s1_ug := s1_#tmp11;
      s1_#tmp12 := to_unit(s1_ug);
      s1_#tmp13 := construct_Row_ipblocks(ipblocks_ipb_user[s1_userId][s1_ipb_id], ipblocks_ipb_id[s1_userId][s1_ipb_id], ipblocks_ipb_address[s1_userId][s1_ipb_id], ipblocks_ipb_expiry[s1_userId][s1_ipb_id]);
      s1_ip := s1_#tmp13;
      s1_#tmp14 := to_unit(s1_ip);
  ...
  // Snapshotting final state for a_then_b
    ...
    s1_pr_type_a_then_b := s1_pr_type;
    s1_latest_rev_id_a_then_b := s1_latest_rev_id;
  // Restoring initial state:
    ...
    s1_ug_group := s1_ug_group_init;
    s1_pr_type := s1_pr_type_init;
    s1_latest_rev_id := s1_latest_rev_id_init;
    s1_ipb_id := s1_ipb_id_init;
    s0_active := true;
    s1_active := true;
  // Executing B then A:
    if (s1_active) {
  ...
  // Snapshotting final state for b_then_a
    ...
    s1_pr_type_b_then_a := s1_pr_type;
    s1_latest_rev_id_b_then_a := s1_latest_rev_id;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 1) (hop_id . 1)) (node_2 (function_id . 18) (instance . 1) (hop_id . 4)))"} ((((((((user_user_editcount_a_then_b == user_user_editcount_b_then_a) && (user_user_id_a_then_b == user_user_id_b_then_a)) && (user_user_name_a_then_b == user_user_name_b_then_a)) && (user_user_touched_a_then_b == user_user_touched_b_then_a)) && (watchlist_wl_notificationtimestamp_a_then_b == watchlist_wl_notificationtimestamp_b_then_a)) && (s1_pageId_a_then_b == s1_pageId_b_then_a)) && (s1_pr_type_a_then_b == s1_pr_type_b_then_a)) && (s1_latest_rev_id_a_then_b == s1_latest_rev_id_b_then_a));
}
