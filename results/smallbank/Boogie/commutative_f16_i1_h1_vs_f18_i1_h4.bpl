...
modifies Accounts_name, Checking_bal, Checking_custid, Savings_bal, Savings_custid;
{
  var s0_custId0 : int;
  var s0_custId1 : int;
  var s1_custId : int;
  ...
  var s0_#tmp2 : real;
  var s0_total : real;
  var s1_#tmp11 : Row (Table (Accounts));
  var s1_a : Row (Table (Accounts));
  var s1_#tmp12 : unit;
  var s1_c#bal : real;
  var s1_c#custid : int;
  var s1_#tmp14 : real;
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block4__ab:
      s1_#tmp11 := construct_Row_Accounts(Accounts_custid[s1_custId], Accounts_name[s1_custId]);
      s1_a := s1_#tmp11;
      s1_#tmp12 := to_unit(s1_a);
      s1_c#bal := Checking_bal[s1_custId];
      s1_c#custid := Checking_custid[s1_custId];
      s1_#tmp14 := s1_c#bal + s1_amount;
      s1_c#bal := s1_#tmp14;
      Checking_bal := Checking_bal[s1_custId := s1_c#bal];
      Checking_custid := Checking_custid[s1_custId := s1_c#custid];
      s1_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 1) (hop_id . 1)) (node_2 (function_id . 18) (instance . 1) (hop_id . 4)))"} ((((((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a)) && (Savings_bal_a_then_b == Savings_bal_b_then_a)) && (Savings_custid_a_then_b == Savings_custid_b_then_a)) && (s0_custId1_a_then_b == s0_custId1_b_then_a)) && (s0_total_a_then_b == s0_total_b_then_a));
}
