...
modifies Accounts_name, Checking_bal, Checking_custid, Savings_bal, Savings_custid;
{
  ...
  var s0_#tmp2 : real;
  var s0_total : real;
  var s1_#tmp5 : Row (Table (Accounts));
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block3__ab:
      s1_#tmp5 := construct_Row_Accounts(Accounts_custid[s1_custId], Accounts_name[s1_custId]);
      s1_a := s1_#tmp5;
      s1_#tmp6 := to_unit(s1_a);
      s1_s#bal := Savings_bal[s1_custId];
      s1_c#bal := Checking_bal[s1_custId];
      s1_#tmp9 := s1_s#bal + s1_c#bal;
      s1_total := s1_#tmp9;
      s1_#tmp10 := to_unit(s1_total);
      s1_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 1) (hop_id . 1)) (node_2 (function_id . 17) (instance . 0) (hop_id . 3)))"} ((((((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a)) && (Savings_bal_a_then_b == Savings_bal_b_then_a)) && (Savings_custid_a_then_b == Savings_custid_b_then_a)) && (s0_custId1_a_then_b == s0_custId1_b_then_a)) && (s0_total_a_then_b == s0_total_b_then_a));
}
