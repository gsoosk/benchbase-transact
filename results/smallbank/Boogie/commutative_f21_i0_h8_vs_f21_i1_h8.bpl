...
modifies Checking_bal, Checking_custid, Savings_bal;
{
  ...
  // Executing A then B:
    ...
    s0_block8__ab:
      ...
      s0_#tmp25 := s0_s#bal + s0_c#bal;
      s0_total := s0_#tmp25;
      s0_#tmp26 := s0_total < s0_amount;
      if (s0_#tmp26) {
      goto s0_block9__ab;
    } else {
      goto s0_block11__ab;
    ...
    s0_block9__ab:
      s0_#tmp27 := s0_c#bal - s0_amount;
      s0_#tmp28 := s0_#tmp27 - 1.0000000;
      s0_c#bal := s0_#tmp28;
      goto s0_block10__ab;
    s0_block10__ab:
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 21) (instance . 0) (hop_id . 8)) (node_2 (function_id . 21) (instance . 1) (hop_id . 8)))"} ((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a));
}
