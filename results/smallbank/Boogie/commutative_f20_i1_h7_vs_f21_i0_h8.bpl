...
procedure Check_SliceCommut_Hop7_vs_Hop8()
modifies Accounts_name, Checking_bal, Checking_custid, Savings_bal, Savings_custid;
{
  ...
  var s0_#tmp19 : Row (Table (Accounts));
  ...
  // Slice commutativity verification: hop 7 vs hop 8
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block7__ab:
      s0_#tmp19 := construct_Row_Accounts(Accounts_custid[s0_custId], Accounts_name[s0_custId]);
      s0_a := s0_#tmp19;
      s0_#tmp20 := to_unit(s0_a);
      s0_s#bal := Savings_bal[s0_custId];
      s0_s#custid := Savings_custid[s0_custId];
      s0_#tmp22 := s0_s#bal - s0_amount;
      s0_s#bal := s0_#tmp22;
      Savings_bal := Savings_bal[s0_custId := s0_s#bal];
      Savings_custid := Savings_custid[s0_custId := s0_s#custid];
      s0_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 20) (instance . 1) (hop_id . 7)) (node_2 (function_id . 21) (instance . 0) (hop_id . 8)))"} ((((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a)) && (Savings_bal_a_then_b == Savings_bal_b_then_a)) && (Savings_custid_a_then_b == Savings_custid_b_then_a));
}
