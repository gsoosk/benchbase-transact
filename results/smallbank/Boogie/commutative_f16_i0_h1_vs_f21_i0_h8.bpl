...
procedure Check_SliceCommut_Hop1_vs_Hop8()
modifies Checking_bal, Checking_custid, Savings_bal, Savings_custid;
{
  ...
  // Slice commutativity verification: hop 1 vs hop 8
  ...
  // Executing A then B:
    ...
    s1_block8__ab:
      ...
      s1_#tmp25 := s1_s#bal + s1_c#bal;
      s1_total := s1_#tmp25;
      s1_#tmp26 := s1_total < s1_amount;
      if (s1_#tmp26) {
      goto s1_block9__ab;
    } else {
      goto s1_block11__ab;
    ...
    s1_block9__ab:
      s1_#tmp27 := s1_c#bal - s1_amount;
      s1_#tmp28 := s1_#tmp27 - 1.0000000;
      s1_c#bal := s1_#tmp28;
      goto s1_block10__ab;
    s1_block10__ab:
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 0) (hop_id . 1)) (node_2 (function_id . 21) (instance . 0) (hop_id . 8)))"} ((((((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a)) && (Savings_bal_a_then_b == Savings_bal_b_then_a)) && (Savings_custid_a_then_b == Savings_custid_b_then_a)) && (s0_custId1_a_then_b == s0_custId1_b_then_a)) && (s0_total_a_then_b == s0_total_b_then_a));
}
