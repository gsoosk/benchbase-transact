...
modifies Accounts_name, Checking_bal, Savings_bal, Savings_custid;
{
  var s0_custId : int;
  var s1_custId : int;
  var s1_amount : real;
  ...
  var s0_#tmp5 : Row (Table (Accounts));
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block3__ab:
      s0_#tmp5 := construct_Row_Accounts(Accounts_custid[s0_custId], Accounts_name[s0_custId]);
      s0_a := s0_#tmp5;
      s0_#tmp6 := to_unit(s0_a);
      s0_s#bal := Savings_bal[s0_custId];
      s0_c#bal := Checking_bal[s0_custId];
      s0_#tmp9 := s0_s#bal + s0_c#bal;
      s0_total := s0_#tmp9;
      s0_#tmp10 := to_unit(s0_total);
      s0_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 17) (instance . 0) (hop_id . 3)) (node_2 (function_id . 20) (instance . 0) (hop_id . 7)))"} ((Savings_bal_a_then_b == Savings_bal_b_then_a) && (Savings_custid_a_then_b == Savings_custid_b_then_a));
}
