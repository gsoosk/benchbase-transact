...
modifies Checking_bal, Checking_custid, Savings_bal;
{
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block6__ab:
      s0_c1#bal := Checking_bal[s0_destAcct];
      s0_c1#custid := Checking_custid[s0_destAcct];
      s0_#tmp18 := s0_c1#bal + s0_amount;
      s0_c1#bal := s0_#tmp18;
      Checking_bal := Checking_bal[s0_destAcct := s0_c1#bal];
      Checking_custid := Checking_custid[s0_destAcct := s0_c1#custid];
      s0_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 19) (instance . 1) (hop_id . 6)) (node_2 (function_id . 21) (instance . 1) (hop_id . 8)))"} ((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a));
}
