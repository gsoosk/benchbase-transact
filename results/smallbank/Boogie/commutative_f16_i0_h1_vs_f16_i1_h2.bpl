...
const __slice__ : int;
procedure Check_SliceCommut_Hop1_vs_Hop2()
modifies Checking_bal, Checking_custid, Savings_bal, Savings_custid;
{
  ...
  // Slice commutativity verification: hop 1 vs hop 2
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block2__ab:
      s1_c1#bal := Checking_bal[s1_custId1];
      s1_c1#custid := Checking_custid[s1_custId1];
      s1_#tmp4 := s1_c1#bal + s1_total;
      s1_c1#bal := s1_#tmp4;
      Checking_bal := Checking_bal[s1_custId1 := s1_c1#bal];
      Checking_custid := Checking_custid[s1_custId1 := s1_c1#custid];
      s1_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 0) (hop_id . 1)) (node_2 (function_id . 16) (instance . 1) (hop_id . 2)))"} ((((((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a)) && (Savings_bal_a_then_b == Savings_bal_b_then_a)) && (Savings_custid_a_then_b == Savings_custid_b_then_a)) && (s0_custId1_a_then_b == s0_custId1_b_then_a)) && (s0_total_a_then_b == s0_total_b_then_a));
}
