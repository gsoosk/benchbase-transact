...
modifies Checking_bal, Checking_custid, Savings_bal;
{
  var s0_sendAcct : int;
  var s0_destAcct : int;
  var s0_amount : real;
  ...
  var s0_sendAcct_init : int;
  var s0_destAcct_init : int;
  ...
  // --- Step 2: Save initial state ---
    ...
    s0_sendAcct_init := s0_sendAcct;
    s0_destAcct_init := s0_destAcct;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block5__ab:
      s0_c0#bal := Checking_bal[s0_sendAcct];
      s0_c0#custid := Checking_custid[s0_sendAcct];
      s0_#tmp16 := s0_c0#bal - s0_amount;
      s0_c0#bal := s0_#tmp16;
      Checking_bal := Checking_bal[s0_sendAcct := s0_c0#bal];
      Checking_custid := Checking_custid[s0_sendAcct := s0_c0#custid];
  ...
  // Snapshotting final state for a_then_b
    ...
    s0_destAcct_a_then_b := s0_destAcct;
  ...
  // Restoring initial state:
    ...
    s0_sendAcct := s0_sendAcct_init;
    s0_destAcct := s0_destAcct_init;
  ...
  // Snapshotting final state for b_then_a
    ...
    s0_destAcct_b_then_a := s0_destAcct;
    s0_amount_b_then_a := s0_amount;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 19) (instance . 0) (hop_id . 5)) (node_2 (function_id . 21) (instance . 0) (hop_id . 8)))"} ((((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a)) && (s0_destAcct_a_then_b == s0_destAcct_b_then_a)) && (s0_amount_a_then_b == s0_amount_b_then_a));
}
