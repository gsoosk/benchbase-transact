...
modifies Checking_bal, Checking_custid, Savings_bal, Savings_custid;
{
  ...
  var Savings_bal_init : [int]real;
  var Savings_custid_init : [int]int;
  var s0_custId0_init : int;
  var s0_custId1_init : int;
  var s1_sendAcct_init : int;
  var s1_destAcct_init : int;
  var s1_amount_init : real;
  var s0_s0#bal : real;
  var s0_s0#custid : int;
  var s0_c0#bal : real;
  var s0_c0#custid : int;
  var s0_#tmp2 : real;
  var s0_total : real;
  var s1_c0#bal : real;
  ...
  var Savings_bal_a_then_b : [int]real;
  var Savings_custid_a_then_b : [int]int;
  var s0_custId1_a_then_b : int;
  var s0_total_a_then_b : real;
  var s1_destAcct_a_then_b : int;
  var s1_amount_a_then_b : real;
  var Checking_bal_b_then_a : [int]real;
  var Checking_custid_b_then_a : [int]int;
  var Savings_bal_b_then_a : [int]real;
  var Savings_custid_b_then_a : [int]int;
  var s0_custId1_b_then_a : int;
  var s0_total_b_then_a : real;
  var s1_destAcct_b_then_a : int;
  ...
  // --- Step 2: Save initial state ---
    ...
    Savings_bal_init := Savings_bal;
    Savings_custid_init := Savings_custid;
    s0_custId0_init := s0_custId0;
    s0_custId1_init := s0_custId1;
    s1_sendAcct_init := s1_sendAcct;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block1__ab:
      s0_s0#bal := Savings_bal[s0_custId0];
      s0_s0#custid := Savings_custid[s0_custId0];
      s0_c0#bal := Checking_bal[s0_custId0];
      s0_c0#custid := Checking_custid[s0_custId0];
      s0_#tmp2 := s0_s0#bal + s0_c0#bal;
      s0_total := s0_#tmp2;
      Savings_bal := Savings_bal[s0_custId0 := 0.0000000];
      Savings_custid := Savings_custid[s0_custId0 := s0_s0#custid];
      Checking_bal := Checking_bal[s0_custId0 := 0.0000000];
      Checking_custid := Checking_custid[s0_custId0 := s0_c0#custid];
      goto s0_hop_exit__ab;
    s0_hop_exit__ab:
    }
    if (s1_active) {
    s1_block5__ab:
      s1_c0#bal := Checking_bal[s1_sendAcct];
      s1_c0#custid := Checking_custid[s1_sendAcct];
      s1_#tmp16 := s1_c0#bal - s1_amount;
      s1_c0#bal := s1_#tmp16;
      Checking_bal := Checking_bal[s1_sendAcct := s1_c0#bal];
      Checking_custid := Checking_custid[s1_sendAcct := s1_c0#custid];
  ...
  // Snapshotting final state for a_then_b
    ...
    Savings_bal_a_then_b := Savings_bal;
    Savings_custid_a_then_b := Savings_custid;
    s0_custId1_a_then_b := s0_custId1;
    s0_total_a_then_b := s0_total;
    s1_destAcct_a_then_b := s1_destAcct;
    s1_amount_a_then_b := s1_amount;
  // Restoring initial state:
    Checking_bal := Checking_bal_init;
    Checking_custid := Checking_custid_init;
    Savings_bal := Savings_bal_init;
    Savings_custid := Savings_custid_init;
    s0_custId0 := s0_custId0_init;
    s0_custId1 := s0_custId1_init;
    s1_sendAcct := s1_sendAcct_init;
  ...
  // Snapshotting final state for b_then_a
    ...
    Savings_bal_b_then_a := Savings_bal;
    Savings_custid_b_then_a := Savings_custid;
    s0_custId1_b_then_a := s0_custId1;
    s0_total_b_then_a := s0_total;
    s1_destAcct_b_then_a := s1_destAcct;
    s1_amount_b_then_a := s1_amount;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 0) (hop_id . 1)) (node_2 (function_id . 19) (instance . 1) (hop_id . 5)))"} ((((((((Checking_bal_a_then_b == Checking_bal_b_then_a) && (Checking_custid_a_then_b == Checking_custid_b_then_a)) && (Savings_bal_a_then_b == Savings_bal_b_then_a)) && (Savings_custid_a_then_b == Savings_custid_b_then_a)) && (s0_custId1_a_then_b == s0_custId1_b_then_a)) && (s0_total_a_then_b == s0_total_b_then_a)) && (s1_destAcct_a_then_b == s1_destAcct_b_then_a)) && (s1_amount_a_then_b == s1_amount_b_then_a));
}
