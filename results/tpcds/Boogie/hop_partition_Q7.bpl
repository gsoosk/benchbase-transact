...
procedure verify_hop_partitions_Q7(item_sk: int, ticket_num: int, cd_demo_sk: int, promo_sk: int)
{
  var s7_ss#ss_list_price : real;
  var s7_item_sk : int;
  var s7_ticket_num : int;
  var s7_ss#ss_quantity : int;
  var s7_#tmp7 : Row (Table (item));
  ...
  var s7_price : real;
  var s7_#tmp10 : unit;
  var s8_cd#cd_gender : String;
  var s8_cd_demo_sk : int;
  var s8_gender : String;
  var s8_#tmp12 : unit;
  var s9_promo#p_promo_name : String;
  var s9_promo_sk : int;
  var s9_pname : String;
  var s9_#tmp14 : unit;

  // Hop partition verification for function 'Q7'
  s7_block7:
    s7_ss#ss_list_price := store_sales_ss_list_price[s7_item_sk][s7_ticket_num];
    s7_ss#ss_quantity := store_sales_ss_quantity[s7_item_sk][s7_ticket_num];
  // Partition check hop 7 func 'f_item' tables 'store_sales'=>'store_sales' keys [k0=item_sk] first_span Span { start: 6581, end: 6643, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpcds.transact" } current_span Span { start: 6581, end: 6643, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpcds.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 20) (hop_id . 7) (table_id . 9) (span ((start . 6581) (end . 6643) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpcds.transact\"))))"} (s7_item_sk == s7_item_sk);
    s7_#tmp7 := construct_Row_item(item_i_item_sk[s7_item_sk], item_i_item_id[s7_item_sk], item_i_item_desc[s7_item_sk], item_i_current_price[s7_item_sk], item_i_class[s7_item_sk], item_i_category[s7_item_sk], item_i_manufact[s7_item_sk]);
  // Partition check hop 7 func 'f_item' tables 'store_sales'=>'item' keys [k0=item_sk] first_span Span { start: 6581, end: 6643, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpcds.transact" } current_span Span { start: 6672, end: 6696, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpcds.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 20) (hop_id . 7) (table_id . 3) (span ((start . 6672) (end . 6696) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpcds.transact\"))))"} (s7_item_sk == s7_item_sk);
    s7_i := s7_#tmp7;
    s7_#tmp8 := to_unit(s7_i);
    s7_qty := s7_ss#ss_quantity;
    s7_#tmp9 := to_unit(s7_qty);
    s7_price := s7_ss#ss_list_price;
    s7_#tmp10 := to_unit(s7_price);
    goto s7_hop_exit;
  s7_hop_exit:
  s8_block8:
    s8_cd#cd_gender := customer_demographics_cd_gender[s8_cd_demo_sk];
    s8_gender := s8_cd#cd_gender;
    s8_#tmp12 := to_unit(s8_gender);
    goto s8_hop_exit;
  s8_hop_exit:
  s9_block9:
    s9_promo#p_promo_name := promotion_p_promo_name[s9_promo_sk];
    s9_pname := s9_promo#p_promo_name;
    s9_#tmp14 := to_unit(s9_pname);
    goto s9_epilogue;
  s9_hop_exit:
  s7_epilogue:
  s8_epilogue:
