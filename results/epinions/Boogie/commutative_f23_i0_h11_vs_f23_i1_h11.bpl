...
modifies review_creation_date, review_i_id, review_rating, review_u_id;
{
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block11__ab:
      s0_r#creation_date := review_creation_date[s0_uid][s0_iid];
      s0_r#i_id := review_i_id[s0_uid][s0_iid];
      s0_r#u_id := review_u_id[s0_uid][s0_iid];
      s0_r#rating := s0_rating;
      review_creation_date := review_creation_date[s0_uid := review_creation_date[s0_uid][s0_iid := s0_r#creation_date]];
      review_i_id := review_i_id[s0_uid := review_i_id[s0_uid][s0_iid := s0_r#i_id]];
      review_rating := review_rating[s0_uid := review_rating[s0_uid][s0_iid := s0_r#rating]];
      review_u_id := review_u_id[s0_uid := review_u_id[s0_uid][s0_iid := s0_r#u_id]];
      s0_active := false;
