...
modifies review_creation_date, review_i_id, review_rating, review_u_id;
{
  ...
  var s0_#tmp16 : Row (Table (review));
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block9__ab:
      s0_#tmp16 := construct_Row_review(review_u_id[s0_uid][s0_iid], review_i_id[s0_uid][s0_iid], review_rating[s0_uid][s0_iid], review_creation_date[s0_uid][s0_iid]);
      s0_r := s0_#tmp16;
      s0_#tmp17 := to_unit(s0_r);
      s0_active := false;
