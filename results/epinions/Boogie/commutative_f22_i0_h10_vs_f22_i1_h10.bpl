...
modifies item_i_id, item_title;
{
  ...
  var s0_iid_init : int;
  var s0_title_init : String;
  var s1_iid_init : int;
  var s1_title_init : String;
  var s0_i#i_id : int;
  var s0_i#title : String;
  var s1_i#i_id : int;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block10__ab:
      s0_i#i_id := item_i_id[s0_iid];
      s0_i#title := s0_title;
      item_i_id := item_i_id[s0_iid := s0_i#i_id];
      item_title := item_title[s0_iid := s0_i#title];
      s0_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 22) (instance . 0) (hop_id . 10)) (node_2 (function_id . 22) (instance . 1) (hop_id . 10)))"} ((item_i_id_a_then_b == item_i_id_b_then_a) && (item_title_a_then_b == item_title_b_then_a));
}
