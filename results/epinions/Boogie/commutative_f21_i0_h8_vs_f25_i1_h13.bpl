...
modifies useracct_name, useracct_u_id;
{
  ...
  var s0_#tmp14 : Row (Table (useracct));
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block8__ab:
      s0_#tmp14 := construct_Row_useracct(useracct_u_id[s0_uid], useracct_name[s0_uid]);
      s0_u := s0_#tmp14;
      s0_#tmp15 := to_unit(s0_u);
  ...
  // Snapshotting final state for b_then_a
    ...
    s0_iid_b_then_a := s0_iid;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 21) (instance . 0) (hop_id . 8)) (node_2 (function_id . 25) (instance . 1) (hop_id . 13)))"} ((((useracct_name_a_then_b == useracct_name_b_then_a) && (useracct_u_id_a_then_b == useracct_u_id_b_then_a)) && (s0_uid_a_then_b == s0_uid_b_then_a)) && (s0_iid_a_then_b == s0_iid_b_then_a));
}
