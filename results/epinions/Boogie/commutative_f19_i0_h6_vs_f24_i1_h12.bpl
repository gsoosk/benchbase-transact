...
modifies trust_source_u_id, trust_target_u_id, trust_trust;
{
  ...
  var s0_#tmp8 : Row (Table (trust));
  var s0_t : Row (Table (trust));
  var s0_#tmp9 : unit;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block6__ab:
      s0_#tmp8 := construct_Row_trust(trust_source_u_id[s0_uid][s0_reviewer_uid], trust_target_u_id[s0_uid][s0_reviewer_uid], trust_trust[s0_uid][s0_reviewer_uid]);
      s0_t := s0_#tmp8;
      s0_#tmp9 := to_unit(s0_t);
      s0_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 19) (instance . 0) (hop_id . 6)) (node_2 (function_id . 24) (instance . 1) (hop_id . 12)))"} (((trust_source_u_id_a_then_b == trust_source_u_id_b_then_a) && (trust_target_u_id_a_then_b == trust_target_u_id_b_then_a)) && (trust_trust_a_then_b == trust_trust_b_then_a));
}
