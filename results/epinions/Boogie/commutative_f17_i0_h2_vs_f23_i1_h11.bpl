...
procedure Check_SliceCommut_Hop2_vs_Hop11()
modifies review_creation_date, review_i_id, review_rating, review_u_id;
{
  ...
  // Slice commutativity verification: hop 2 vs hop 11
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block2__ab:
      s0_r#rating := review_rating[s0_reviewer_uid][s0_iid];
      s0_rating := s0_r#rating;
      s0_#tmp1 := to_unit(s0_rating);
