...
modifies review_creation_date, review_i_id, review_rating, review_u_id;
{
  var s0_iid : int;
  var s0_uid : int;
  var s0_reviewer_uid : int;
  var s1_iid : int;
  var s1_uid : int;
  var s1_rating : int;
  var s0_active : bool;
  var s1_active : bool;
  var review_creation_date_init : [int][int]int;
  var review_i_id_init : [int][int]int;
  var review_rating_init : [int][int]int;
  var review_u_id_init : [int][int]int;
  var s0_iid_init : int;
  var s0_uid_init : int;
  var s0_reviewer_uid_init : int;
  var s1_iid_init : int;
  var s1_uid_init : int;
  var s1_rating_init : int;
  var s0_#tmp6 : Row (Table (review));
  var s0_r : Row (Table (review));
  var s0_#tmp7 : unit;
  var s1_r#creation_date : int;
  var s1_r#i_id : int;
  var s1_r#u_id : int;
  var s1_r#rating : int;
  var review_creation_date_a_then_b : [int][int]int;
  var review_i_id_a_then_b : [int][int]int;
  var review_rating_a_then_b : [int][int]int;
  var review_u_id_a_then_b : [int][int]int;
  var s0_uid_a_then_b : int;
  var s0_reviewer_uid_a_then_b : int;
  var review_creation_date_b_then_a : [int][int]int;
  var review_i_id_b_then_a : [int][int]int;
  var review_rating_b_then_a : [int][int]int;
  var review_u_id_b_then_a : [int][int]int;
  var s0_uid_b_then_a : int;
  var s0_reviewer_uid_b_then_a : int;

  // Slice commutativity verification: hop 5 vs hop 11
  // --- Step 1: Havoc initial state ---
    havoc review_creation_date;
    havoc review_i_id;
    havoc review_rating;
    havoc review_u_id;
    havoc s0_iid;
    havoc s0_uid;
    havoc s0_reviewer_uid;
    havoc s1_iid;
    havoc s1_uid;
    havoc s1_rating;
    s0_active := true;
  ...
  // --- Step 2: Save initial state ---
    review_creation_date_init := review_creation_date;
    review_i_id_init := review_i_id;
    review_rating_init := review_rating;
    review_u_id_init := review_u_id;
    s0_iid_init := s0_iid;
    s0_uid_init := s0_uid;
    s0_reviewer_uid_init := s0_reviewer_uid;
    s1_iid_init := s1_iid;
    s1_uid_init := s1_uid;
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block5__ab:
      s0_#tmp6 := construct_Row_review(review_u_id[s0_reviewer_uid][s0_iid], review_i_id[s0_reviewer_uid][s0_iid], review_rating[s0_reviewer_uid][s0_iid], review_creation_date[s0_reviewer_uid][s0_iid]);
      s0_r := s0_#tmp6;
      s0_#tmp7 := to_unit(s0_r);
      goto s0_hop_exit__ab;
    s0_hop_exit__ab:
    }
    if (s1_active) {
    s1_block11__ab:
      s1_r#creation_date := review_creation_date[s1_uid][s1_iid];
      s1_r#i_id := review_i_id[s1_uid][s1_iid];
      s1_r#u_id := review_u_id[s1_uid][s1_iid];
      s1_r#rating := s1_rating;
      review_creation_date := review_creation_date[s1_uid := review_creation_date[s1_uid][s1_iid := s1_r#creation_date]];
      review_i_id := review_i_id[s1_uid := review_i_id[s1_uid][s1_iid := s1_r#i_id]];
      review_rating := review_rating[s1_uid := review_rating[s1_uid][s1_iid := s1_r#rating]];
      review_u_id := review_u_id[s1_uid := review_u_id[s1_uid][s1_iid := s1_r#u_id]];
      s1_active := false;
      goto s1_hop_exit__ab;
    s1_hop_exit__ab:
    }
  // Snapshotting final state for a_then_b
    review_creation_date_a_then_b := review_creation_date;
    review_i_id_a_then_b := review_i_id;
    review_rating_a_then_b := review_rating;
    review_u_id_a_then_b := review_u_id;
    s0_uid_a_then_b := s0_uid;
    s0_reviewer_uid_a_then_b := s0_reviewer_uid;
  // Restoring initial state:
    review_creation_date := review_creation_date_init;
    review_i_id := review_i_id_init;
    review_rating := review_rating_init;
    review_u_id := review_u_id_init;
    s0_iid := s0_iid_init;
    s0_uid := s0_uid_init;
    s0_reviewer_uid := s0_reviewer_uid_init;
    s1_iid := s1_iid_init;
    s1_uid := s1_uid_init;
  ...
  // Snapshotting final state for b_then_a
    review_creation_date_b_then_a := review_creation_date;
    review_i_id_b_then_a := review_i_id;
    review_rating_b_then_a := review_rating;
    review_u_id_b_then_a := review_u_id;
    s0_uid_b_then_a := s0_uid;
    s0_reviewer_uid_b_then_a := s0_reviewer_uid;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 19) (instance . 0) (hop_id . 5)) (node_2 (function_id . 23) (instance . 0) (hop_id . 11)))"} ((((((review_creation_date_a_then_b == review_creation_date_b_then_a) && (review_i_id_a_then_b == review_i_id_b_then_a)) && (review_rating_a_then_b == review_rating_b_then_a)) && (review_u_id_a_then_b == review_u_id_b_then_a)) && (s0_uid_a_then_b == s0_uid_b_then_a)) && (s0_reviewer_uid_a_then_b == s0_reviewer_uid_b_then_a));
}
