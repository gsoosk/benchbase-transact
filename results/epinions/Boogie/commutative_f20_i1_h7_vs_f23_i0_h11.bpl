...
procedure Check_SliceCommut_Hop7_vs_Hop11()
modifies item_title, review_creation_date, review_i_id, review_rating, review_u_id;
{
  var s0_iid : int;
  ...
  var s0_active : bool;
  var s1_active : bool;
  var item_title_init : [int]String;
  ...
  var s0_#tmp10 : Row (Table (review));
  var s0_r : Row (Table (review));
  var s0_#tmp11 : unit;
  var s0_#tmp12 : Row (Table (item));
  var s0_i : Row (Table (item));
  var s0_#tmp13 : unit;
  ...
  // Slice commutativity verification: hop 7 vs hop 11
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block7__ab:
      s0_#tmp10 := construct_Row_review(review_u_id[s0_reviewer_uid][s0_iid], review_i_id[s0_reviewer_uid][s0_iid], review_rating[s0_reviewer_uid][s0_iid], review_creation_date[s0_reviewer_uid][s0_iid]);
      s0_r := s0_#tmp10;
      s0_#tmp11 := to_unit(s0_r);
      s0_#tmp12 := construct_Row_item(item_i_id[s0_iid], item_title[s0_iid]);
      s0_i := s0_#tmp12;
      s0_#tmp13 := to_unit(s0_i);
      s0_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 20) (instance . 1) (hop_id . 7)) (node_2 (function_id . 23) (instance . 0) (hop_id . 11)))"} ((((review_creation_date_a_then_b == review_creation_date_b_then_a) && (review_i_id_a_then_b == review_i_id_b_then_a)) && (review_rating_a_then_b == review_rating_b_then_a)) && (review_u_id_a_then_b == review_u_id_b_then_a));
}
