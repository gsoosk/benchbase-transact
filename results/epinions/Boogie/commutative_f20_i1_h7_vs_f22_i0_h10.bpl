...
modifies item_i_id, item_title, review_creation_date, review_rating;
{
  var s0_iid : int;
  ...
  var s0_#tmp13 : unit;
  var s1_i#i_id : int;
  var s1_i#title : String;
  var item_i_id_a_then_b : [int]int;
  var item_title_a_then_b : [int]String;
  var item_i_id_b_then_a : [int]int;
  var item_title_b_then_a : [int]String;
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block10__ab:
      s1_i#i_id := item_i_id[s1_iid];
      s1_i#title := s1_title;
      item_i_id := item_i_id[s1_iid := s1_i#i_id];
      item_title := item_title[s1_iid := s1_i#title];
      s1_active := false;
  ...
  // Snapshotting final state for a_then_b
    ...
    item_title_a_then_b := item_title;
  ...
  // Snapshotting final state for b_then_a
    ...
    item_title_b_then_a := item_title;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 20) (instance . 1) (hop_id . 7)) (node_2 (function_id . 22) (instance . 0) (hop_id . 10)))"} ((item_i_id_a_then_b == item_i_id_b_then_a) && (item_title_a_then_b == item_title_b_then_a));
}
