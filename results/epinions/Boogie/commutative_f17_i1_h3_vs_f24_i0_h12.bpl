...
modifies trust_source_u_id, trust_target_u_id, trust_trust;
{
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block3__ab:
      s0_t#trust := trust_trust[s0_uid][s0_reviewer_uid];
      s0_trust_level := s0_t#trust;
      s0_#tmp3 := to_unit(s0_trust_level);
      s0_active := false;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 17) (instance . 1) (hop_id . 3)) (node_2 (function_id . 24) (instance . 0) (hop_id . 12)))"} (((trust_source_u_id_a_then_b == trust_source_u_id_b_then_a) && (trust_target_u_id_a_then_b == trust_target_u_id_b_then_a)) && (trust_trust_a_then_b == trust_trust_b_then_a));
}
