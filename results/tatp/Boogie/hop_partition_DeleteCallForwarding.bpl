...
procedure verify_hop_partitions_DeleteCallForwarding(s_id: int, sf_type: int, start_time: int)
{
  var s1_#tmp0 : Row (Table (CallForwarding));
  var s1_s_id : int;
  var s1_sf_type : int;
  var s1_start_time : int;
  var s1_cf : Row (Table (CallForwarding));
  var s1_#tmp1 : unit;

  // Hop partition verification for function 'DeleteCallForwarding'
  s1_block1:
    s1_#tmp0 := construct_Row_CallForwarding(CallForwarding_s_id[s1_s_id][s1_sf_type][s1_start_time], CallForwarding_sf_type[s1_s_id][s1_sf_type][s1_start_time], CallForwarding_start_time[s1_s_id][s1_sf_type][s1_start_time], CallForwarding_end_time[s1_s_id][s1_sf_type][s1_start_time], CallForwarding_numberx[s1_s_id][s1_sf_type][s1_start_time]);
    s1_cf := s1_#tmp0;
    s1_#tmp1 := to_unit(s1_cf);
    goto s1_epilogue;
  s1_hop_exit:
  s1_epilogue:
}
