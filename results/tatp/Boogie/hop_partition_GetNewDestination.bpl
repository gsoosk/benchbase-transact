...
procedure verify_hop_partitions_GetNewDestination(s_id: int, sf_type: int, start_time: int, end_time: int)
{
  var s3_sf#is_active : int;
  var s3_s_id : int;
  var s3_sf_type : int;
  var s3_sf_active : int;
  var s3_#tmp5 : bool;
  var s3_cf#numberx : String;
  var s3_start_time : int;
  var s3_num : String;
  var s3_#tmp7 : unit;

  // Hop partition verification for function 'GetNewDestination'
  s3_block3:
    s3_sf#is_active := SpecialFacility_is_active[s3_s_id][s3_sf_type];
    s3_sf_active := s3_sf#is_active;
    s3_#tmp5 := s3_sf_active == 1;
    if (s3_#tmp5) {
      goto s3_block4;
    } else {
      goto s3_block5;
  ...
  s3_block4:
    s3_cf#numberx := CallForwarding_numberx[s3_s_id][s3_sf_type][s3_start_time];
  // Partition check hop 3 func 'subPartition' tables 'SpecialFacility'=>'CallForwarding' keys [k0=s_id] first_span Span { start: 2341, end: 2386, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tatp.transact" } current_span Span { start: 2501, end: 2569, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tatp.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 18) (hop_id . 3) (table_id . 3) (span ((start . 2501) (end . 2569) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tatp.transact\"))))"} (s3_s_id == s3_s_id);
    s3_num := s3_cf#numberx;
    s3_#tmp7 := to_unit(s3_num);
    goto s3_block5;
  s3_block5:
    goto s3_epilogue;
  s3_hop_exit:
