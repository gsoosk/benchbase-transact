...
procedure verify_hop_partitions_GetSubscriberData(s_id: int)
{
  var s4_sub#bit_1 : int;
  var s4_s_id : int;
  var s4_bit1 : int;
  var s4_#tmp9 : unit;

  // Hop partition verification for function 'GetSubscriberData'
  s4_block6:
    s4_sub#bit_1 := Subscriber_bit_1[s4_s_id];
    s4_bit1 := s4_sub#bit_1;
    s4_#tmp9 := to_unit(s4_bit1);
    goto s4_epilogue;
  s4_hop_exit:
  s4_epilogue:
}
