...
procedure Check_SliceCommut_Hop201_vs_Hop201()
modifies locktable_empid, locktable_salary;
{
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block201__ab:
      s0_r1#empid := locktable_empid[s0_key1];
      s0_r1#salary := s0_newSalary;
      locktable_empid := locktable_empid[s0_key1 := s0_r1#empid];
      locktable_salary := locktable_salary[s0_key1 := s0_r1#salary];
      goto s0_hop_exit__ab;
    s0_hop_exit__ab:
    }
    if (s1_active) {
    s1_block201__ab:
      s1_r1#empid := locktable_empid[s1_key1];
      s1_r1#salary := s1_newSalary;
      locktable_empid := locktable_empid[s1_key1 := s1_r1#empid];
      locktable_salary := locktable_salary[s1_key1 := s1_r1#salary];
  ...
  // Snapshotting final state for a_then_b
    ...
    s0_newSalary_a_then_b := s0_newSalary;
    s1_key2_a_then_b := s1_key2;
    s1_newSalary_a_then_b := s1_newSalary;
  ...
  // Snapshotting final state for b_then_a
    ...
    s0_newSalary_b_then_a := s0_newSalary;
    s1_key2_b_then_a := s1_key2;
    s1_newSalary_b_then_a := s1_newSalary;
  // --- Step 4: Verify A→B ≡ B→A (Special interleavings equivalence) ---
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 18) (instance . 0) (hop_id . 201)) (node_2 (function_id . 18) (instance . 1) (hop_id . 201)))"} ((((((locktable_empid_a_then_b == locktable_empid_b_then_a) && (locktable_salary_a_then_b == locktable_salary_b_then_a)) && (s0_key2_a_then_b == s0_key2_b_then_a)) && (s0_newSalary_a_then_b == s0_newSalary_b_then_a)) && (s1_key2_a_then_b == s1_key2_b_then_a)) && (s1_newSalary_a_then_b == s1_newSalary_b_then_a));
}
