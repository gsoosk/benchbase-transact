...
procedure Check_SliceCommut_Hop203_vs_Hop203()
modifies locktable_empid, locktable_salary;
{
  ...
  // Slice commutativity verification: hop 203 vs hop 203
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block203__ab:
      s0_r1#empid := locktable_empid[s0_leftKey];
      s0_r1#salary := s0_newSalary;
      locktable_empid := locktable_empid[s0_leftKey := s0_r1#empid];
      locktable_salary := locktable_salary[s0_leftKey := s0_r1#salary];
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 19) (instance . 0) (hop_id . 203)) (node_2 (function_id . 19) (instance . 1) (hop_id . 203)))"} ((((((locktable_empid_a_then_b == locktable_empid_b_then_a) && (locktable_salary_a_then_b == locktable_salary_b_then_a)) && (s0_leftKey_a_then_b == s0_leftKey_b_then_a)) && (s0_newSalary_a_then_b == s0_newSalary_b_then_a)) && (s1_leftKey_a_then_b == s1_leftKey_b_then_a)) && (s1_newSalary_a_then_b == s1_newSalary_b_then_a));
}
