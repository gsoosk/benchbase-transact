...
{
  var s0_representative_id : int;
  var s1_id : int;
  var s0_active : bool;
  var s1_active : bool;
  var SITable_id_init : [int]int;
  var SITable_value_init : [int]int;
  var s0_representative_id_init : int;
  var s1_id_init : int;
  var s0_r#value : int;
  var s0_v : int;
  var s0_#tmp1 : unit;
  ...
  // --- Step 2: Save initial state ---
    ...
    s0_representative_id_init := s0_representative_id;
    s1_id_init := s1_id;
  // --- Step 3: Execute special interleavings ---
  // Executing A then B:
    if (s0_active) {
    s0_block1__ab:
      s0_r#value := SITable_value[s0_representative_id];
      s0_v := s0_r#value;
      s0_#tmp1 := to_unit(s0_v);
      s0_active := false;
  ...
  // Restoring initial state:
    ...
    s0_representative_id := s0_representative_id_init;
    s1_id := s1_id_init;
    s0_active := true;
    s1_active := true;
  // Executing B then A:
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 16) (instance . 0) (hop_id . 1)) (node_2 (function_id . 17) (instance . 0) (hop_id . 2)))"} ((SITable_id_a_then_b == SITable_id_b_then_a) && (SITable_value_a_then_b == SITable_value_b_then_a));
}
