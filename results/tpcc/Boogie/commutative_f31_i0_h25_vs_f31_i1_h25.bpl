...
procedure Check_SliceCommut_Hop25_vs_Hop25()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 25 vs hop 25
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block65__ab:
      s1_#tmp216 := get(s1_itemIDs, 8);
      s1_i_id := s1_#tmp216;
      s1_#tmp217 := get(s1_supplierWarehouseIDs, 8);
      s1_sup_w_id := s1_#tmp217;
      s1_#tmp218 := get(s1_orderQuantities, 8);
      s1_qty := s1_#tmp218;
      s1_#tmp219 := s1_qty >= 0;
      s1_#tmp220 := s1_qty <= 10;
      s1_#tmp221 := s1_#tmp219 && s1_#tmp220;
      ...
      s1_#tmp223 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp223;
      s1_#tmp224 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp224;
      s1_#tmp225 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp226 := s1_#tmp225 >= 10;
      if (s1_#tmp226) {
      goto s1_block66__ab;
    } else {
      goto s1_block68__ab;
    ...
    s1_block66__ab:
      s1_#tmp227 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp227;
      goto s1_block67__ab;
    s1_block67__ab:
      s1_#tmp230 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp230) {
      goto s1_block69__ab;
    } else {
      goto s1_block70__ab;
    ...
    s1_block68__ab:
      s1_#tmp228 := s1_s#S_QUANTITY + 91;
      s1_#tmp229 := s1_#tmp228 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp229;
      goto s1_block67__ab;
    s1_block69__ab:
      s1_#tmp231 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp231;
      goto s1_block70__ab;
    s1_block70__ab:
      s1_#tmp232 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp232;
      s1_#tmp233 := real(s1_qty);
      s1_#tmp234 := s1_s#S_YTD + s1_#tmp233;
      s1_s#S_YTD := s1_#tmp234;
