...
procedure Check_SliceCommut_Hop24_vs_Hop26()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 24 vs hop 26
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block59__ab:
      s0_#tmp197 := get(s0_itemIDs, 7);
      s0_i_id := s0_#tmp197;
      s0_#tmp198 := get(s0_supplierWarehouseIDs, 7);
      s0_sup_w_id := s0_#tmp198;
      s0_#tmp199 := get(s0_orderQuantities, 7);
      s0_qty := s0_#tmp199;
      s0_#tmp200 := s0_qty >= 0;
      s0_#tmp201 := s0_qty <= 10;
      s0_#tmp202 := s0_#tmp200 && s0_#tmp201;
      ...
      s0_#tmp204 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp204;
      s0_#tmp205 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp205;
      s0_#tmp206 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp207 := s0_#tmp206 >= 10;
      if (s0_#tmp207) {
      goto s0_block60__ab;
    } else {
      goto s0_block62__ab;
    ...
    s0_block60__ab:
      s0_#tmp208 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp208;
      goto s0_block61__ab;
    s0_block61__ab:
      s0_#tmp211 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp211) {
      goto s0_block63__ab;
    } else {
      goto s0_block64__ab;
    ...
    s0_block62__ab:
      s0_#tmp209 := s0_s#S_QUANTITY + 91;
      s0_#tmp210 := s0_#tmp209 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp210;
      goto s0_block61__ab;
    s0_block63__ab:
      s0_#tmp212 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp212;
      goto s0_block64__ab;
    s0_block64__ab:
      s0_#tmp213 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp213;
      s0_#tmp214 := real(s0_qty);
      s0_#tmp215 := s0_s#S_YTD + s0_#tmp214;
      s0_s#S_YTD := s0_#tmp215;
