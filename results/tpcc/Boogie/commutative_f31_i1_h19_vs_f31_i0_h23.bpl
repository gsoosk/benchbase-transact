...
procedure Check_SliceCommut_Hop19_vs_Hop23()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 19 vs hop 23
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block29__ab:
      s0_#tmp102 := get(s0_itemIDs, 2);
      s0_i_id := s0_#tmp102;
      s0_#tmp103 := get(s0_supplierWarehouseIDs, 2);
      s0_sup_w_id := s0_#tmp103;
      s0_#tmp104 := get(s0_orderQuantities, 2);
      s0_qty := s0_#tmp104;
      s0_#tmp105 := s0_qty >= 0;
      s0_#tmp106 := s0_qty <= 10;
      s0_#tmp107 := s0_#tmp105 && s0_#tmp106;
      ...
      s0_#tmp109 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp109;
      s0_#tmp110 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp110;
      s0_#tmp111 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp112 := s0_#tmp111 >= 10;
      if (s0_#tmp112) {
      goto s0_block30__ab;
    } else {
      goto s0_block32__ab;
    ...
    s0_block30__ab:
      s0_#tmp113 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp113;
      goto s0_block31__ab;
    s0_block31__ab:
      s0_#tmp116 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp116) {
      goto s0_block33__ab;
    } else {
      goto s0_block34__ab;
    ...
    s0_block32__ab:
      s0_#tmp114 := s0_s#S_QUANTITY + 91;
      s0_#tmp115 := s0_#tmp114 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp115;
      goto s0_block31__ab;
    s0_block33__ab:
      s0_#tmp117 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp117;
      goto s0_block34__ab;
    s0_block34__ab:
      s0_#tmp118 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp118;
      s0_#tmp119 := real(s0_qty);
      s0_#tmp120 := s0_s#S_YTD + s0_#tmp119;
      s0_s#S_YTD := s0_#tmp120;
    ...
    if (s1_active) {
    s1_block53__ab:
      s1_#tmp178 := get(s1_itemIDs, 6);
      s1_i_id := s1_#tmp178;
      s1_#tmp179 := get(s1_supplierWarehouseIDs, 6);
      s1_sup_w_id := s1_#tmp179;
      s1_#tmp180 := get(s1_orderQuantities, 6);
      s1_qty := s1_#tmp180;
      s1_#tmp181 := s1_qty >= 0;
      s1_#tmp182 := s1_qty <= 10;
      s1_#tmp183 := s1_#tmp181 && s1_#tmp182;
      ...
      s1_#tmp185 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp185;
      s1_#tmp186 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp186;
      s1_#tmp187 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp188 := s1_#tmp187 >= 10;
      if (s1_#tmp188) {
      goto s1_block54__ab;
    } else {
      goto s1_block56__ab;
    ...
    s1_block54__ab:
      s1_#tmp189 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp189;
      goto s1_block55__ab;
    s1_block55__ab:
      s1_#tmp192 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp192) {
      goto s1_block57__ab;
    } else {
      goto s1_block58__ab;
    ...
    s1_block56__ab:
      s1_#tmp190 := s1_s#S_QUANTITY + 91;
      s1_#tmp191 := s1_#tmp190 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp191;
      goto s1_block55__ab;
    s1_block57__ab:
      s1_#tmp193 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp193;
      goto s1_block58__ab;
    s1_block58__ab:
      s1_#tmp194 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp194;
      s1_#tmp195 := real(s1_qty);
      s1_#tmp196 := s1_s#S_YTD + s1_#tmp195;
      s1_s#S_YTD := s1_#tmp196;
