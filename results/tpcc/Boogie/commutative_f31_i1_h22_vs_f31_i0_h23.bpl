...
procedure Check_SliceCommut_Hop22_vs_Hop23()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 22 vs hop 23
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block47__ab:
      s0_#tmp159 := get(s0_itemIDs, 5);
      s0_i_id := s0_#tmp159;
      s0_#tmp160 := get(s0_supplierWarehouseIDs, 5);
      s0_sup_w_id := s0_#tmp160;
      s0_#tmp161 := get(s0_orderQuantities, 5);
      s0_qty := s0_#tmp161;
      s0_#tmp162 := s0_qty >= 0;
      s0_#tmp163 := s0_qty <= 10;
      s0_#tmp164 := s0_#tmp162 && s0_#tmp163;
      ...
      s0_#tmp166 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp166;
      s0_#tmp167 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp167;
      s0_#tmp168 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp169 := s0_#tmp168 >= 10;
      if (s0_#tmp169) {
      goto s0_block48__ab;
    } else {
      goto s0_block50__ab;
    ...
    s0_block48__ab:
      s0_#tmp170 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp170;
      goto s0_block49__ab;
    s0_block49__ab:
      s0_#tmp173 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp173) {
      goto s0_block51__ab;
    } else {
      goto s0_block52__ab;
    ...
    s0_block50__ab:
      s0_#tmp171 := s0_s#S_QUANTITY + 91;
      s0_#tmp172 := s0_#tmp171 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp172;
      goto s0_block49__ab;
    s0_block51__ab:
      s0_#tmp174 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp174;
      goto s0_block52__ab;
    s0_block52__ab:
      s0_#tmp175 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp175;
      s0_#tmp176 := real(s0_qty);
      s0_#tmp177 := s0_s#S_YTD + s0_#tmp176;
      s0_s#S_YTD := s0_#tmp177;
