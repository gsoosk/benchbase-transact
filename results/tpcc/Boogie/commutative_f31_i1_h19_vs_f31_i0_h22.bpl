...
procedure Check_SliceCommut_Hop19_vs_Hop22()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 19 vs hop 22
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block47__ab:
      s1_#tmp159 := get(s1_itemIDs, 5);
      s1_i_id := s1_#tmp159;
      s1_#tmp160 := get(s1_supplierWarehouseIDs, 5);
      s1_sup_w_id := s1_#tmp160;
      s1_#tmp161 := get(s1_orderQuantities, 5);
      s1_qty := s1_#tmp161;
      s1_#tmp162 := s1_qty >= 0;
      s1_#tmp163 := s1_qty <= 10;
      s1_#tmp164 := s1_#tmp162 && s1_#tmp163;
      ...
      s1_#tmp166 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp166;
      s1_#tmp167 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp167;
      s1_#tmp168 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp169 := s1_#tmp168 >= 10;
      if (s1_#tmp169) {
      goto s1_block48__ab;
    } else {
      goto s1_block50__ab;
    ...
    s1_block48__ab:
      s1_#tmp170 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp170;
      goto s1_block49__ab;
    s1_block49__ab:
      s1_#tmp173 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp173) {
      goto s1_block51__ab;
    } else {
      goto s1_block52__ab;
    ...
    s1_block50__ab:
      s1_#tmp171 := s1_s#S_QUANTITY + 91;
      s1_#tmp172 := s1_#tmp171 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp172;
      goto s1_block49__ab;
    s1_block51__ab:
      s1_#tmp174 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp174;
      goto s1_block52__ab;
    s1_block52__ab:
      s1_#tmp175 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp175;
      s1_#tmp176 := real(s1_qty);
      s1_#tmp177 := s1_s#S_YTD + s1_#tmp176;
      s1_s#S_YTD := s1_#tmp177;
