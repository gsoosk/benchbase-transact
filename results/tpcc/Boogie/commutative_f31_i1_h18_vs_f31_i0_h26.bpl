...
procedure Check_SliceCommut_Hop18_vs_Hop26()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 18 vs hop 26
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block23__ab:
      s0_#tmp83 := get(s0_itemIDs, 1);
      s0_i_id := s0_#tmp83;
      s0_#tmp84 := get(s0_supplierWarehouseIDs, 1);
      s0_sup_w_id := s0_#tmp84;
      s0_#tmp85 := get(s0_orderQuantities, 1);
      s0_qty := s0_#tmp85;
      s0_#tmp86 := s0_qty >= 0;
      s0_#tmp87 := s0_qty <= 10;
      s0_#tmp88 := s0_#tmp86 && s0_#tmp87;
      ...
      s0_#tmp90 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp90;
      s0_#tmp91 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp91;
      s0_#tmp92 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp93 := s0_#tmp92 >= 10;
      if (s0_#tmp93) {
      goto s0_block24__ab;
    } else {
      goto s0_block26__ab;
    ...
    s0_block24__ab:
      s0_#tmp94 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp94;
      goto s0_block25__ab;
    s0_block25__ab:
      s0_#tmp97 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp97) {
      goto s0_block27__ab;
    } else {
      goto s0_block28__ab;
    ...
    s0_block26__ab:
      s0_#tmp95 := s0_s#S_QUANTITY + 91;
      s0_#tmp96 := s0_#tmp95 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp96;
      goto s0_block25__ab;
    s0_block27__ab:
      s0_#tmp98 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp98;
      goto s0_block28__ab;
    s0_block28__ab:
      s0_#tmp99 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp99;
      s0_#tmp100 := real(s0_qty);
      s0_#tmp101 := s0_s#S_YTD + s0_#tmp100;
      s0_s#S_YTD := s0_#tmp101;
