...
procedure Check_SliceCommut_Hop42_vs_Hop42()
modifies District_D_YTD;
{
  var s0_w_id : int;
  var s0_d_id : int;
  var s0_c_w_id : int;
  ...
  var s0_c_w_id_a_then_b : int;
  var s0_c_d_id_a_then_b : int;
  var s0_c_id_a_then_b : int;
  var s0_amount_a_then_b : real;
  var s1_w_id_a_then_b : int;
  var s1_d_id_a_then_b : int;
  var s1_c_w_id_a_then_b : int;
  var s1_c_d_id_a_then_b : int;
  ...
  var s0_c_w_id_b_then_a : int;
  var s0_c_d_id_b_then_a : int;
  var s0_c_id_b_then_a : int;
  var s0_amount_b_then_a : real;
  var s1_w_id_b_then_a : int;
  var s1_d_id_b_then_a : int;
  var s1_c_w_id_b_then_a : int;
  var s1_c_d_id_b_then_a : int;
  var s1_c_id_b_then_a : int;
  var s1_amount_b_then_a : real;

  // Slice commutativity verification: hop 42 vs hop 42
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block144__ab:
      s0_#tmp425 := District_D_YTD[s0_w_id][s0_d_id];
      s0_#tmp426 := s0_#tmp425 + s0_amount;
      District_D_YTD := District_D_YTD[s0_w_id := District_D_YTD[s0_w_id][s0_d_id := s0_#tmp426]];
      goto s0_hop_exit__ab;
    s0_hop_exit__ab:
    }
    if (s1_active) {
    s1_block144__ab:
      s1_#tmp425 := District_D_YTD[s1_w_id][s1_d_id];
      s1_#tmp426 := s1_#tmp425 + s1_amount;
      District_D_YTD := District_D_YTD[s1_w_id := District_D_YTD[s1_w_id][s1_d_id := s1_#tmp426]];
  ...
  // Snapshotting final state for a_then_b
    ...
    s0_c_w_id_a_then_b := s0_c_w_id;
    s0_c_d_id_a_then_b := s0_c_d_id;
    s0_c_id_a_then_b := s0_c_id;
    s0_amount_a_then_b := s0_amount;
    s1_w_id_a_then_b := s1_w_id;
    s1_d_id_a_then_b := s1_d_id;
    s1_c_w_id_a_then_b := s1_c_w_id;
    s1_c_d_id_a_then_b := s1_c_d_id;
  ...
  // Snapshotting final state for b_then_a
    ...
    s0_c_w_id_b_then_a := s0_c_w_id;
    s0_c_d_id_b_then_a := s0_c_d_id;
    s0_c_id_b_then_a := s0_c_id;
    s0_amount_b_then_a := s0_amount;
    s1_w_id_b_then_a := s1_w_id;
    s1_d_id_b_then_a := s1_d_id;
    s1_c_w_id_b_then_a := s1_c_w_id;
    s1_c_d_id_b_then_a := s1_c_d_id;
  ...
  // Verifying A->B === B->A equivalence:
    assert {:msg "(SpecialInterleavingNonEquivalence (node_1 (function_id . 35) (instance . 0) (hop_id . 42)) (node_2 (function_id . 35) (instance . 1) (hop_id . 42)))"} (((((((((((((District_D_YTD_a_then_b == District_D_YTD_b_then_a) && (s0_w_id_a_then_b == s0_w_id_b_then_a)) && (s0_d_id_a_then_b == s0_d_id_b_then_a)) && (s0_c_w_id_a_then_b == s0_c_w_id_b_then_a)) && (s0_c_d_id_a_then_b == s0_c_d_id_b_then_a)) && (s0_c_id_a_then_b == s0_c_id_b_then_a)) && (s0_amount_a_then_b == s0_amount_b_then_a)) && (s1_w_id_a_then_b == s1_w_id_b_then_a)) && (s1_d_id_a_then_b == s1_d_id_b_then_a)) && (s1_c_w_id_a_then_b == s1_c_w_id_b_then_a)) && (s1_c_d_id_a_then_b == s1_c_d_id_b_then_a)) && (s1_c_id_a_then_b == s1_c_id_b_then_a)) && (s1_amount_a_then_b == s1_amount_b_then_a));
}
