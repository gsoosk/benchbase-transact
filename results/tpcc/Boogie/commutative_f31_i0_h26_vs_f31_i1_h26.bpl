...
procedure Check_SliceCommut_Hop26_vs_Hop26()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 26 vs hop 26
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block71__ab:
      s0_#tmp235 := get(s0_itemIDs, 9);
      s0_i_id := s0_#tmp235;
      s0_#tmp236 := get(s0_supplierWarehouseIDs, 9);
      s0_sup_w_id := s0_#tmp236;
      s0_#tmp237 := get(s0_orderQuantities, 9);
      s0_qty := s0_#tmp237;
      s0_#tmp238 := s0_qty >= 0;
      s0_#tmp239 := s0_qty <= 10;
      s0_#tmp240 := s0_#tmp238 && s0_#tmp239;
      ...
      s0_#tmp242 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp242;
      s0_#tmp243 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp243;
      s0_#tmp244 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp245 := s0_#tmp244 >= 10;
      if (s0_#tmp245) {
      goto s0_block72__ab;
    } else {
      goto s0_block74__ab;
    ...
    s0_block72__ab:
      s0_#tmp246 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp246;
      goto s0_block73__ab;
    s0_block73__ab:
      s0_#tmp249 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp249) {
      goto s0_block75__ab;
    } else {
      goto s0_block76__ab;
    ...
    s0_block74__ab:
      s0_#tmp247 := s0_s#S_QUANTITY + 91;
      s0_#tmp248 := s0_#tmp247 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp248;
      goto s0_block73__ab;
    s0_block75__ab:
      s0_#tmp250 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp250;
      goto s0_block76__ab;
    s0_block76__ab:
      s0_#tmp251 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp251;
      s0_#tmp252 := real(s0_qty);
      s0_#tmp253 := s0_s#S_YTD + s0_#tmp252;
      s0_s#S_YTD := s0_#tmp253;
