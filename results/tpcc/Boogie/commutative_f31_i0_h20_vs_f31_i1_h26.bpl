...
procedure Check_SliceCommut_Hop20_vs_Hop26()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 20 vs hop 26
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block71__ab:
      s1_#tmp235 := get(s1_itemIDs, 9);
      s1_i_id := s1_#tmp235;
      s1_#tmp236 := get(s1_supplierWarehouseIDs, 9);
      s1_sup_w_id := s1_#tmp236;
      s1_#tmp237 := get(s1_orderQuantities, 9);
      s1_qty := s1_#tmp237;
      s1_#tmp238 := s1_qty >= 0;
      s1_#tmp239 := s1_qty <= 10;
      s1_#tmp240 := s1_#tmp238 && s1_#tmp239;
      ...
      s1_#tmp242 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp242;
      s1_#tmp243 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp243;
      s1_#tmp244 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp245 := s1_#tmp244 >= 10;
      if (s1_#tmp245) {
      goto s1_block72__ab;
    } else {
      goto s1_block74__ab;
    ...
    s1_block72__ab:
      s1_#tmp246 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp246;
      goto s1_block73__ab;
    s1_block73__ab:
      s1_#tmp249 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp249) {
      goto s1_block75__ab;
    } else {
      goto s1_block76__ab;
    ...
    s1_block74__ab:
      s1_#tmp247 := s1_s#S_QUANTITY + 91;
      s1_#tmp248 := s1_#tmp247 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp248;
      goto s1_block73__ab;
    s1_block75__ab:
      s1_#tmp250 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp250;
      goto s1_block76__ab;
    s1_block76__ab:
      s1_#tmp251 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp251;
      s1_#tmp252 := real(s1_qty);
      s1_#tmp253 := s1_s#S_YTD + s1_#tmp252;
      s1_s#S_YTD := s1_#tmp253;
