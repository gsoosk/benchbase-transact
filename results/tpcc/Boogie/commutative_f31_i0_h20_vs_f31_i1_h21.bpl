...
procedure Check_SliceCommut_Hop20_vs_Hop21()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 20 vs hop 21
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block35__ab:
      s0_#tmp121 := get(s0_itemIDs, 3);
      s0_i_id := s0_#tmp121;
      s0_#tmp122 := get(s0_supplierWarehouseIDs, 3);
      s0_sup_w_id := s0_#tmp122;
      s0_#tmp123 := get(s0_orderQuantities, 3);
      s0_qty := s0_#tmp123;
      s0_#tmp124 := s0_qty >= 0;
      s0_#tmp125 := s0_qty <= 10;
      s0_#tmp126 := s0_#tmp124 && s0_#tmp125;
      ...
      s0_#tmp128 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp128;
      s0_#tmp129 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp129;
      s0_#tmp130 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp131 := s0_#tmp130 >= 10;
      if (s0_#tmp131) {
      goto s0_block36__ab;
    } else {
      goto s0_block38__ab;
    ...
    s0_block36__ab:
      s0_#tmp132 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp132;
      goto s0_block37__ab;
    s0_block37__ab:
      s0_#tmp135 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp135) {
      goto s0_block39__ab;
    } else {
      goto s0_block40__ab;
    ...
    s0_block38__ab:
      s0_#tmp133 := s0_s#S_QUANTITY + 91;
      s0_#tmp134 := s0_#tmp133 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp134;
      goto s0_block37__ab;
    s0_block39__ab:
      s0_#tmp136 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp136;
      goto s0_block40__ab;
    s0_block40__ab:
      s0_#tmp137 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp137;
      s0_#tmp138 := real(s0_qty);
      s0_#tmp139 := s0_s#S_YTD + s0_#tmp138;
      s0_s#S_YTD := s0_#tmp139;
    ...
    if (s1_active) {
    s1_block41__ab:
      s1_#tmp140 := get(s1_itemIDs, 4);
      s1_i_id := s1_#tmp140;
      s1_#tmp141 := get(s1_supplierWarehouseIDs, 4);
      s1_sup_w_id := s1_#tmp141;
      s1_#tmp142 := get(s1_orderQuantities, 4);
      s1_qty := s1_#tmp142;
      s1_#tmp143 := s1_qty >= 0;
      s1_#tmp144 := s1_qty <= 10;
      s1_#tmp145 := s1_#tmp143 && s1_#tmp144;
      ...
      s1_#tmp147 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp147;
      s1_#tmp148 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp148;
      s1_#tmp149 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp150 := s1_#tmp149 >= 10;
      if (s1_#tmp150) {
      goto s1_block42__ab;
    } else {
      goto s1_block44__ab;
    ...
    s1_block42__ab:
      s1_#tmp151 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp151;
      goto s1_block43__ab;
    s1_block43__ab:
      s1_#tmp154 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp154) {
      goto s1_block45__ab;
    } else {
      goto s1_block46__ab;
    ...
    s1_block44__ab:
      s1_#tmp152 := s1_s#S_QUANTITY + 91;
      s1_#tmp153 := s1_#tmp152 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp153;
      goto s1_block43__ab;
    s1_block45__ab:
      s1_#tmp155 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp155;
      goto s1_block46__ab;
    s1_block46__ab:
      s1_#tmp156 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp156;
      s1_#tmp157 := real(s1_qty);
      s1_#tmp158 := s1_s#S_YTD + s1_#tmp157;
      s1_s#S_YTD := s1_#tmp158;
