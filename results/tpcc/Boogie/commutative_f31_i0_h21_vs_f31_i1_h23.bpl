...
procedure Check_SliceCommut_Hop21_vs_Hop23()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 21 vs hop 23
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block41__ab:
      s0_#tmp140 := get(s0_itemIDs, 4);
      s0_i_id := s0_#tmp140;
      s0_#tmp141 := get(s0_supplierWarehouseIDs, 4);
      s0_sup_w_id := s0_#tmp141;
      s0_#tmp142 := get(s0_orderQuantities, 4);
      s0_qty := s0_#tmp142;
      s0_#tmp143 := s0_qty >= 0;
      s0_#tmp144 := s0_qty <= 10;
      s0_#tmp145 := s0_#tmp143 && s0_#tmp144;
      ...
      s0_#tmp147 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp147;
      s0_#tmp148 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp148;
      s0_#tmp149 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp150 := s0_#tmp149 >= 10;
      if (s0_#tmp150) {
      goto s0_block42__ab;
    } else {
      goto s0_block44__ab;
    ...
    s0_block42__ab:
      s0_#tmp151 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp151;
      goto s0_block43__ab;
    s0_block43__ab:
      s0_#tmp154 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp154) {
      goto s0_block45__ab;
    } else {
      goto s0_block46__ab;
    ...
    s0_block44__ab:
      s0_#tmp152 := s0_s#S_QUANTITY + 91;
      s0_#tmp153 := s0_#tmp152 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp153;
      goto s0_block43__ab;
    s0_block45__ab:
      s0_#tmp155 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp155;
      goto s0_block46__ab;
    s0_block46__ab:
      s0_#tmp156 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp156;
      s0_#tmp157 := real(s0_qty);
      s0_#tmp158 := s0_s#S_YTD + s0_#tmp157;
      s0_s#S_YTD := s0_#tmp158;
