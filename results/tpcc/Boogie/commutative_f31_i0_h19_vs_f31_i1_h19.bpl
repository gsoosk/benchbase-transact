...
procedure Check_SliceCommut_Hop19_vs_Hop19()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 19 vs hop 19
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block29__ab:
      s1_#tmp102 := get(s1_itemIDs, 2);
      s1_i_id := s1_#tmp102;
      s1_#tmp103 := get(s1_supplierWarehouseIDs, 2);
      s1_sup_w_id := s1_#tmp103;
      s1_#tmp104 := get(s1_orderQuantities, 2);
      s1_qty := s1_#tmp104;
      s1_#tmp105 := s1_qty >= 0;
      s1_#tmp106 := s1_qty <= 10;
      s1_#tmp107 := s1_#tmp105 && s1_#tmp106;
      ...
      s1_#tmp109 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp109;
      s1_#tmp110 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp110;
      s1_#tmp111 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp112 := s1_#tmp111 >= 10;
      if (s1_#tmp112) {
      goto s1_block30__ab;
    } else {
      goto s1_block32__ab;
    ...
    s1_block30__ab:
      s1_#tmp113 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp113;
      goto s1_block31__ab;
    s1_block31__ab:
      s1_#tmp116 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp116) {
      goto s1_block33__ab;
    } else {
      goto s1_block34__ab;
    ...
    s1_block32__ab:
      s1_#tmp114 := s1_s#S_QUANTITY + 91;
      s1_#tmp115 := s1_#tmp114 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp115;
      goto s1_block31__ab;
    s1_block33__ab:
      s1_#tmp117 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp117;
      goto s1_block34__ab;
    s1_block34__ab:
      s1_#tmp118 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp118;
      s1_#tmp119 := real(s1_qty);
      s1_#tmp120 := s1_s#S_YTD + s1_#tmp119;
      s1_s#S_YTD := s1_#tmp120;
