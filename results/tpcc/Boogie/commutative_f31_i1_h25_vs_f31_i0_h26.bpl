...
procedure Check_SliceCommut_Hop25_vs_Hop26()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 25 vs hop 26
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block65__ab:
      s0_#tmp216 := get(s0_itemIDs, 8);
      s0_i_id := s0_#tmp216;
      s0_#tmp217 := get(s0_supplierWarehouseIDs, 8);
      s0_sup_w_id := s0_#tmp217;
      s0_#tmp218 := get(s0_orderQuantities, 8);
      s0_qty := s0_#tmp218;
      s0_#tmp219 := s0_qty >= 0;
      s0_#tmp220 := s0_qty <= 10;
      s0_#tmp221 := s0_#tmp219 && s0_#tmp220;
      ...
      s0_#tmp223 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp223;
      s0_#tmp224 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp224;
      s0_#tmp225 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp226 := s0_#tmp225 >= 10;
      if (s0_#tmp226) {
      goto s0_block66__ab;
    } else {
      goto s0_block68__ab;
    ...
    s0_block66__ab:
      s0_#tmp227 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp227;
      goto s0_block67__ab;
    s0_block67__ab:
      s0_#tmp230 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp230) {
      goto s0_block69__ab;
    } else {
      goto s0_block70__ab;
    ...
    s0_block68__ab:
      s0_#tmp228 := s0_s#S_QUANTITY + 91;
      s0_#tmp229 := s0_#tmp228 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp229;
      goto s0_block67__ab;
    s0_block69__ab:
      s0_#tmp231 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp231;
      goto s0_block70__ab;
    s0_block70__ab:
      s0_#tmp232 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp232;
      s0_#tmp233 := real(s0_qty);
      s0_#tmp234 := s0_s#S_YTD + s0_#tmp233;
      s0_s#S_YTD := s0_#tmp234;
