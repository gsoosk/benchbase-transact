...
procedure Check_SliceCommut_Hop19_vs_Hop20()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 19 vs hop 20
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block35__ab:
      s1_#tmp121 := get(s1_itemIDs, 3);
      s1_i_id := s1_#tmp121;
      s1_#tmp122 := get(s1_supplierWarehouseIDs, 3);
      s1_sup_w_id := s1_#tmp122;
      s1_#tmp123 := get(s1_orderQuantities, 3);
      s1_qty := s1_#tmp123;
      s1_#tmp124 := s1_qty >= 0;
      s1_#tmp125 := s1_qty <= 10;
      s1_#tmp126 := s1_#tmp124 && s1_#tmp125;
      ...
      s1_#tmp128 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp128;
      s1_#tmp129 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp129;
      s1_#tmp130 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp131 := s1_#tmp130 >= 10;
      if (s1_#tmp131) {
      goto s1_block36__ab;
    } else {
      goto s1_block38__ab;
    ...
    s1_block36__ab:
      s1_#tmp132 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp132;
      goto s1_block37__ab;
    s1_block37__ab:
      s1_#tmp135 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp135) {
      goto s1_block39__ab;
    } else {
      goto s1_block40__ab;
    ...
    s1_block38__ab:
      s1_#tmp133 := s1_s#S_QUANTITY + 91;
      s1_#tmp134 := s1_#tmp133 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp134;
      goto s1_block37__ab;
    s1_block39__ab:
      s1_#tmp136 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp136;
      goto s1_block40__ab;
    s1_block40__ab:
      s1_#tmp137 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp137;
      s1_#tmp138 := real(s1_qty);
      s1_#tmp139 := s1_s#S_YTD + s1_#tmp138;
      s1_s#S_YTD := s1_#tmp139;
