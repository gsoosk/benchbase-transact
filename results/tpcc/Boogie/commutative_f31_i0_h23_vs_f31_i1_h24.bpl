...
procedure Check_SliceCommut_Hop23_vs_Hop24()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 23 vs hop 24
  ...
  // Executing A then B:
    if (s0_active) {
    s0_block53__ab:
      s0_#tmp178 := get(s0_itemIDs, 6);
      s0_i_id := s0_#tmp178;
      s0_#tmp179 := get(s0_supplierWarehouseIDs, 6);
      s0_sup_w_id := s0_#tmp179;
      s0_#tmp180 := get(s0_orderQuantities, 6);
      s0_qty := s0_#tmp180;
      s0_#tmp181 := s0_qty >= 0;
      s0_#tmp182 := s0_qty <= 10;
      s0_#tmp183 := s0_#tmp181 && s0_#tmp182;
      ...
      s0_#tmp185 := Item_I_PRICE[s0_i_id][s0_sup_w_id];
      s0_price := s0_#tmp185;
      s0_#tmp186 := append(s0_prices, s0_price);
      s0_prices := s0_#tmp186;
      s0_#tmp187 := s0_s#S_QUANTITY - s0_qty;
      s0_#tmp188 := s0_#tmp187 >= 10;
      if (s0_#tmp188) {
      goto s0_block54__ab;
    } else {
      goto s0_block56__ab;
    ...
    s0_block54__ab:
      s0_#tmp189 := s0_s#S_QUANTITY - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp189;
      goto s0_block55__ab;
    s0_block55__ab:
      s0_#tmp192 := s0_sup_w_id != s0_w_id;
      if (s0_#tmp192) {
      goto s0_block57__ab;
    } else {
      goto s0_block58__ab;
    ...
    s0_block56__ab:
      s0_#tmp190 := s0_s#S_QUANTITY + 91;
      s0_#tmp191 := s0_#tmp190 - s0_qty;
      s0_s#S_QUANTITY := s0_#tmp191;
      goto s0_block55__ab;
    s0_block57__ab:
      s0_#tmp193 := s0_s#S_REMOTE_CNT + 1;
      s0_s#S_REMOTE_CNT := s0_#tmp193;
      goto s0_block58__ab;
    s0_block58__ab:
      s0_#tmp194 := s0_s#S_ORDER_CNT + 1;
      s0_s#S_ORDER_CNT := s0_#tmp194;
      s0_#tmp195 := real(s0_qty);
      s0_#tmp196 := s0_s#S_YTD + s0_#tmp195;
      s0_s#S_YTD := s0_#tmp196;
    ...
    if (s1_active) {
    s1_block59__ab:
      s1_#tmp197 := get(s1_itemIDs, 7);
      s1_i_id := s1_#tmp197;
      s1_#tmp198 := get(s1_supplierWarehouseIDs, 7);
      s1_sup_w_id := s1_#tmp198;
      s1_#tmp199 := get(s1_orderQuantities, 7);
      s1_qty := s1_#tmp199;
      s1_#tmp200 := s1_qty >= 0;
      s1_#tmp201 := s1_qty <= 10;
      s1_#tmp202 := s1_#tmp200 && s1_#tmp201;
      ...
      s1_#tmp204 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp204;
      s1_#tmp205 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp205;
      s1_#tmp206 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp207 := s1_#tmp206 >= 10;
      if (s1_#tmp207) {
      goto s1_block60__ab;
    } else {
      goto s1_block62__ab;
    ...
    s1_block60__ab:
      s1_#tmp208 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp208;
      goto s1_block61__ab;
    s1_block61__ab:
      s1_#tmp211 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp211) {
      goto s1_block63__ab;
    } else {
      goto s1_block64__ab;
    ...
    s1_block62__ab:
      s1_#tmp209 := s1_s#S_QUANTITY + 91;
      s1_#tmp210 := s1_#tmp209 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp210;
      goto s1_block61__ab;
    s1_block63__ab:
      s1_#tmp212 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp212;
      goto s1_block64__ab;
    s1_block64__ab:
      s1_#tmp213 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp213;
      s1_#tmp214 := real(s1_qty);
      s1_#tmp215 := s1_s#S_YTD + s1_#tmp214;
      s1_s#S_YTD := s1_#tmp215;
