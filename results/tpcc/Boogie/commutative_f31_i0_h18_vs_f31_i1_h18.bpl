...
procedure Check_SliceCommut_Hop18_vs_Hop18()
modifies Item_I_PRICE, Stock_S_DATA, Stock_S_I_ID, Stock_S_ORDER_CNT, Stock_S_QUANTITY, Stock_S_REMOTE_CNT, Stock_S_W_ID, Stock_S_YTD;
{
  ...
  // Slice commutativity verification: hop 18 vs hop 18
  ...
  // Executing A then B:
    ...
    if (s1_active) {
    s1_block23__ab:
      s1_#tmp83 := get(s1_itemIDs, 1);
      s1_i_id := s1_#tmp83;
      s1_#tmp84 := get(s1_supplierWarehouseIDs, 1);
      s1_sup_w_id := s1_#tmp84;
      s1_#tmp85 := get(s1_orderQuantities, 1);
      s1_qty := s1_#tmp85;
      s1_#tmp86 := s1_qty >= 0;
      s1_#tmp87 := s1_qty <= 10;
      s1_#tmp88 := s1_#tmp86 && s1_#tmp87;
      ...
      s1_#tmp90 := Item_I_PRICE[s1_i_id][s1_sup_w_id];
      s1_price := s1_#tmp90;
      s1_#tmp91 := append(s1_prices, s1_price);
      s1_prices := s1_#tmp91;
      s1_#tmp92 := s1_s#S_QUANTITY - s1_qty;
      s1_#tmp93 := s1_#tmp92 >= 10;
      if (s1_#tmp93) {
      goto s1_block24__ab;
    } else {
      goto s1_block26__ab;
    ...
    s1_block24__ab:
      s1_#tmp94 := s1_s#S_QUANTITY - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp94;
      goto s1_block25__ab;
    s1_block25__ab:
      s1_#tmp97 := s1_sup_w_id != s1_w_id;
      if (s1_#tmp97) {
      goto s1_block27__ab;
    } else {
      goto s1_block28__ab;
    ...
    s1_block26__ab:
      s1_#tmp95 := s1_s#S_QUANTITY + 91;
      s1_#tmp96 := s1_#tmp95 - s1_qty;
      s1_s#S_QUANTITY := s1_#tmp96;
      goto s1_block25__ab;
    s1_block27__ab:
      s1_#tmp98 := s1_s#S_REMOTE_CNT + 1;
      s1_s#S_REMOTE_CNT := s1_#tmp98;
      goto s1_block28__ab;
    s1_block28__ab:
      s1_#tmp99 := s1_s#S_ORDER_CNT + 1;
      s1_s#S_ORDER_CNT := s1_#tmp99;
      s1_#tmp100 := real(s1_qty);
      s1_#tmp101 := s1_s#S_YTD + s1_#tmp100;
      s1_s#S_YTD := s1_#tmp101;
