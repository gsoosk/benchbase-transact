...
type Q22Chain struct {
	db   *bolt.DB
	hops []*Hop
}

// Q22Hop0 lowers hop 0 containing 1 basic block(s).
func Q22Hop0(tx *bolt.Tx, in *proto.TrxReq) (*proto.TrxRes, error) {
...
// Q22Hop1 lowers hop 1 containing 1 basic block(s).
func Q22Hop1(tx *bolt.Tx, in *proto.TrxReq) (*proto.TrxRes, error) {
	var rwSet []*proto.RWSet
	params := in.Params
...
func Q22NextReq(in *proto.TrxRes, params map[string]string, shards int) (*proto.TrxReq, map[string]string, int) {
	hopId := in.Info.Hopid
	...
	case 0:
		...
		nextShard = int(Q22Hop0Par(paramsCopy))
	case 1:
		// Deep copy params to avoid modification
		paramsCopy := make(map[string]string)
		for k, v := range params {
		...
		// Calculate partition for next hop using partition function
		nextShard = int(Q22Hop1Par(paramsCopy))
	default:
		return nil, params, 0
	}
...
func Q22ChainImpl(db *bolt.DB) *Chain {
	...
	// add field names
	return &Chain{db: db, hops: hops, NextReq: Q22NextReq}
}

// legacy: use json package to avoid unused import error
func Q22ToJSON(v interface{}) ([]byte, error) {
