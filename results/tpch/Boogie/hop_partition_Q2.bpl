...
procedure verify_hop_partitions_Q2(partkey: int, suppkey: int, nationkey: int, regionkey: int)
{
  var s3_ps#ps_supplycost : real;
  var s3_partkey : int;
  var s3_suppkey : int;
  var s3_#tmp5 : Row (Table (Part));
  var s3_p : Row (Table (Part));
  var s3_#tmp6 : unit;
  var s3_cost : real;
  var s3_#tmp7 : unit;
  var s4_s#s_name : String;
  var s4_suppkey : int;
  var s4_sname : String;
  var s4_#tmp9 : unit;
  var s5_#tmp10 : Row (Table (Nation));
  var s5_nationkey : int;
  var s5_n : Row (Table (Nation));
  var s5_#tmp11 : unit;
  var s5_#tmp12 : Row (Table (Region));
  var s5_regionkey : int;
  var s5_r : Row (Table (Region));
  var s5_#tmp13 : unit;

  // Hop partition verification for function 'Q2'
  s3_block3:
    s3_ps#ps_supplycost := Partsupp_ps_supplycost[s3_partkey][s3_suppkey];
    s3_#tmp5 := construct_Row_Part(Part_p_partkey[s3_partkey], Part_p_name[s3_partkey], Part_p_mfgr[s3_partkey], Part_p_brand[s3_partkey], Part_p_type[s3_partkey], Part_p_size[s3_partkey], Part_p_container[s3_partkey], Part_p_retailprice[s3_partkey], Part_p_comment[s3_partkey]);
  // Partition check hop 3 func 'f' tables 'Partsupp'=>'Part' keys [k0=partkey] first_span Span { start: 4220, end: 4270, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 4299, end: 4323, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 18) (hop_id . 3) (table_id . 2) (span ((start . 4299) (end . 4323) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s3_partkey == s3_partkey);
    s3_p := s3_#tmp5;
    s3_#tmp6 := to_unit(s3_p);
    s3_cost := s3_ps#ps_supplycost;
    s3_#tmp7 := to_unit(s3_cost);
    goto s3_hop_exit;
  s3_hop_exit:
  s4_block4:
    s4_s#s_name := Supplier_s_name[s4_suppkey];
    s4_sname := s4_s#s_name;
    s4_#tmp9 := to_unit(s4_sname);
    goto s4_hop_exit;
  s4_hop_exit:
  s5_block5:
    s5_#tmp10 := construct_Row_Nation(Nation_n_nationkey[s5_nationkey], Nation_n_name[s5_nationkey], Nation_n_regionkey[s5_nationkey], Nation_n_comment[s5_nationkey]);
    s5_n := s5_#tmp10;
    s5_#tmp11 := to_unit(s5_n);
    s5_#tmp12 := construct_Row_Region(Region_r_regionkey[s5_regionkey], Region_r_name[s5_regionkey], Region_r_comment[s5_regionkey]);
  // Partition check hop 5 func 'g' tables 'Nation'=>'Region' keys [k0=nationkey] first_span Span { start: 4596, end: 4626, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 4677, end: 4707, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 16) (function_id . 18) (hop_id . 5) (table_id . 0) (span ((start . 4677) (end . 4707) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s5_nationkey == s5_regionkey);
    s5_r := s5_#tmp12;
    s5_#tmp13 := to_unit(s5_r);
    goto s5_epilogue;
  s5_hop_exit:
  s3_epilogue:
  s4_epilogue:
