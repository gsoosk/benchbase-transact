...
procedure verify_hop_partitions_Q21(suppkey: int, orderkey: int, linenumber: int, nationkey: int)
{
  var s38_#tmp108 : Row (Table (Supplier));
  var s38_suppkey : int;
  var s38_s : Row (Table (Supplier));
  var s38_#tmp109 : unit;
  var s39_#tmp110 : Row (Table (Lineitem));
  var s39_orderkey : int;
  var s39_linenumber : int;
  var s39_l : Row (Table (Lineitem));
  var s39_#tmp111 : unit;
  var s39_#tmp112 : Row (Table (Orders));
  var s39_o : Row (Table (Orders));
  var s39_#tmp113 : unit;
  var s40_#tmp114 : Row (Table (Nation));
  var s40_nationkey : int;
  var s40_n : Row (Table (Nation));
  var s40_#tmp115 : unit;

  // Hop partition verification for function 'Q21'
  s38_block38:
    s38_#tmp108 := construct_Row_Supplier(Supplier_s_suppkey[s38_suppkey], Supplier_s_name[s38_suppkey], Supplier_s_address[s38_suppkey], Supplier_s_nationkey[s38_suppkey], Supplier_s_phone[s38_suppkey], Supplier_s_acctbal[s38_suppkey], Supplier_s_comment[s38_suppkey]);
    s38_s := s38_#tmp108;
    s38_#tmp109 := to_unit(s38_s);
    goto s38_hop_exit;
  s38_hop_exit:
  s39_block39:
    s39_#tmp110 := construct_Row_Lineitem(Lineitem_l_orderkey[s39_orderkey][s39_linenumber], Lineitem_l_linenumber[s39_orderkey][s39_linenumber], Lineitem_l_partkey[s39_orderkey][s39_linenumber], Lineitem_l_suppkey[s39_orderkey][s39_linenumber], Lineitem_l_quantity[s39_orderkey][s39_linenumber], Lineitem_l_extendedprice[s39_orderkey][s39_linenumber], Lineitem_l_discount[s39_orderkey][s39_linenumber], Lineitem_l_tax[s39_orderkey][s39_linenumber], Lineitem_l_returnflag[s39_orderkey][s39_linenumber], Lineitem_l_linestatus[s39_orderkey][s39_linenumber], Lineitem_l_shipdate[s39_orderkey][s39_linenumber], Lineitem_l_commitdate[s39_orderkey][s39_linenumber], Lineitem_l_receiptdate[s39_orderkey][s39_linenumber], Lineitem_l_shipinstruct[s39_orderkey][s39_linenumber], Lineitem_l_shipmode[s39_orderkey][s39_linenumber], Lineitem_l_comment[s39_orderkey][s39_linenumber]);
    s39_l := s39_#tmp110;
    s39_#tmp111 := to_unit(s39_l);
    s39_#tmp112 := construct_Row_Orders(Orders_o_orderkey[s39_orderkey], Orders_o_custkey[s39_orderkey], Orders_o_orderstatus[s39_orderkey], Orders_o_totalprice[s39_orderkey], Orders_o_orderdate[s39_orderkey], Orders_o_orderpriority[s39_orderkey], Orders_o_clerk[s39_orderkey], Orders_o_shippriority[s39_orderkey], Orders_o_comment[s39_orderkey]);
  // Partition check hop 39 func 'f' tables 'Lineitem'=>'Orders' keys [k0=orderkey] first_span Span { start: 12838, end: 12894, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 12945, end: 12973, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 33) (hop_id . 39) (table_id . 6) (span ((start . 12945) (end . 12973) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s39_orderkey == s39_orderkey);
    s39_o := s39_#tmp112;
    s39_#tmp113 := to_unit(s39_o);
    goto s39_hop_exit;
  s39_hop_exit:
  s40_block40:
    s40_#tmp114 := construct_Row_Nation(Nation_n_nationkey[s40_nationkey], Nation_n_name[s40_nationkey], Nation_n_regionkey[s40_nationkey], Nation_n_comment[s40_nationkey]);
    s40_n := s40_#tmp114;
    s40_#tmp115 := to_unit(s40_n);
    goto s40_epilogue;
  s40_hop_exit:
  s38_epilogue:
  s39_epilogue:
