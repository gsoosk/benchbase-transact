...
procedure verify_hop_partitions_Q11(partkey: int, suppkey: int, nationkey: int)
{
  var s16_ps#ps_availqty : int;
  var s16_partkey : int;
  var s16_suppkey : int;
  var s16_ps#ps_supplycost : real;
  var s16_#tmp51 : real;
  var s16_#tmp52 : real;
  var s16_value : real;
  var s16_#tmp53 : unit;
  var s17_#tmp54 : Row (Table (Supplier));
  var s17_suppkey : int;
  var s17_s : Row (Table (Supplier));
  var s17_#tmp55 : unit;
  var s18_#tmp56 : Row (Table (Nation));
  var s18_nationkey : int;
  var s18_n : Row (Table (Nation));
  var s18_#tmp57 : unit;

  // Hop partition verification for function 'Q11'
  s16_block16:
    s16_ps#ps_availqty := Partsupp_ps_availqty[s16_partkey][s16_suppkey];
    s16_ps#ps_supplycost := Partsupp_ps_supplycost[s16_partkey][s16_suppkey];
  // Partition check hop 16 func 'f' tables 'Partsupp'=>'Partsupp' keys [k0=partkey] first_span Span { start: 7697, end: 7747, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 7697, end: 7747, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 23) (hop_id . 16) (table_id . 4) (span ((start . 7697) (end . 7747) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s16_partkey == s16_partkey);
    s16_#tmp51 := real(s16_ps#ps_availqty);
    s16_#tmp52 := s16_#tmp51 * s16_ps#ps_supplycost;
    s16_value := s16_#tmp52;
    s16_#tmp53 := to_unit(s16_value);
    goto s16_hop_exit;
  s16_hop_exit:
  s17_block17:
    s17_#tmp54 := construct_Row_Supplier(Supplier_s_suppkey[s17_suppkey], Supplier_s_name[s17_suppkey], Supplier_s_address[s17_suppkey], Supplier_s_nationkey[s17_suppkey], Supplier_s_phone[s17_suppkey], Supplier_s_acctbal[s17_suppkey], Supplier_s_comment[s17_suppkey]);
    s17_s := s17_#tmp54;
    s17_#tmp55 := to_unit(s17_s);
    goto s17_hop_exit;
  s17_hop_exit:
  s18_block18:
    s18_#tmp56 := construct_Row_Nation(Nation_n_nationkey[s18_nationkey], Nation_n_name[s18_nationkey], Nation_n_regionkey[s18_nationkey], Nation_n_comment[s18_nationkey]);
    s18_n := s18_#tmp56;
    s18_#tmp57 := to_unit(s18_n);
    goto s18_epilogue;
  s18_hop_exit:
  s16_epilogue:
  s17_epilogue:
