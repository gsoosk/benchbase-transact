...
procedure verify_hop_partitions_Q22(custkey: int, orderkey: int)
{
  var s41_c#c_acctbal : real;
  var s41_custkey : int;
  var s41_c#c_phone : String;
  var s41_acctbal : real;
  var s41_#tmp117 : unit;
  var s41_phone : String;
  var s41_#tmp118 : unit;
  var s42_#tmp119 : Row (Table (Orders));
  var s42_orderkey : int;
  var s42_o : Row (Table (Orders));
  var s42_#tmp120 : unit;

  // Hop partition verification for function 'Q22'
  s41_block41:
    s41_c#c_acctbal := Customer_c_acctbal[s41_custkey];
    s41_c#c_phone := Customer_c_phone[s41_custkey];
  // Partition check hop 41 func 'f' tables 'Customer'=>'Customer' keys [k0=custkey] first_span Span { start: 13277, end: 13305, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 13277, end: 13305, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 34) (hop_id . 41) (table_id . 5) (span ((start . 13277) (end . 13305) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s41_custkey == s41_custkey);
    s41_acctbal := s41_c#c_acctbal;
    s41_#tmp117 := to_unit(s41_acctbal);
    s41_phone := s41_c#c_phone;
    s41_#tmp118 := to_unit(s41_phone);
    goto s41_hop_exit;
  s41_hop_exit:
  s42_block42:
    s42_#tmp119 := construct_Row_Orders(Orders_o_orderkey[s42_orderkey], Orders_o_custkey[s42_orderkey], Orders_o_orderstatus[s42_orderkey], Orders_o_totalprice[s42_orderkey], Orders_o_orderdate[s42_orderkey], Orders_o_orderpriority[s42_orderkey], Orders_o_clerk[s42_orderkey], Orders_o_shippriority[s42_orderkey], Orders_o_comment[s42_orderkey]);
    s42_o := s42_#tmp119;
    s42_#tmp120 := to_unit(s42_o);
    goto s42_epilogue;
  s42_hop_exit:
  s41_epilogue:
  s42_epilogue:
