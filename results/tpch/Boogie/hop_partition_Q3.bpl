...
procedure verify_hop_partitions_Q3(custkey: int, orderkey: int, linenumber: int)
{
  var s6_c#c_mktsegment : String;
  var s6_custkey : int;
  var s6_seg : String;
  var s6_#tmp15 : unit;
  var s7_#tmp16 : Row (Table (Orders));
  var s7_orderkey : int;
  var s7_o : Row (Table (Orders));
  var s7_#tmp17 : unit;
  var s7_l#l_discount : real;
  var s7_linenumber : int;
  var s7_l#l_extendedprice : real;
  var s7_#tmp19 : real;
  var s7_#tmp20 : real;
  var s7_revenue : real;
  var s7_#tmp21 : unit;

  // Hop partition verification for function 'Q3'
  s6_block6:
    s6_c#c_mktsegment := Customer_c_mktsegment[s6_custkey];
    s6_seg := s6_c#c_mktsegment;
    s6_#tmp15 := to_unit(s6_seg);
    goto s6_hop_exit;
  s6_hop_exit:
  s7_block7:
    s7_#tmp16 := construct_Row_Orders(Orders_o_orderkey[s7_orderkey], Orders_o_custkey[s7_orderkey], Orders_o_orderstatus[s7_orderkey], Orders_o_totalprice[s7_orderkey], Orders_o_orderdate[s7_orderkey], Orders_o_orderpriority[s7_orderkey], Orders_o_clerk[s7_orderkey], Orders_o_shippriority[s7_orderkey], Orders_o_comment[s7_orderkey]);
    s7_o := s7_#tmp16;
    s7_#tmp17 := to_unit(s7_o);
    s7_l#l_discount := Lineitem_l_discount[s7_orderkey][s7_linenumber];
  // Partition check hop 7 func 'f' tables 'Orders'=>'Lineitem' keys [k0=orderkey] first_span Span { start: 5142, end: 5170, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 5223, end: 5279, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 19) (hop_id . 7) (table_id . 7) (span ((start . 5223) (end . 5279) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s7_orderkey == s7_orderkey);
    s7_l#l_extendedprice := Lineitem_l_extendedprice[s7_orderkey][s7_linenumber];
  // Partition check hop 7 func 'f' tables 'Orders'=>'Lineitem' keys [k0=orderkey] first_span Span { start: 5142, end: 5170, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 5223, end: 5279, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 19) (hop_id . 7) (table_id . 7) (span ((start . 5223) (end . 5279) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s7_orderkey == s7_orderkey);
    s7_#tmp19 := 1.0000000 - s7_l#l_discount;
    s7_#tmp20 := s7_l#l_extendedprice * s7_#tmp19;
    s7_revenue := s7_#tmp20;
    s7_#tmp21 := to_unit(s7_revenue);
    goto s7_epilogue;
  s7_hop_exit:
  s6_epilogue:
  s7_epilogue:
