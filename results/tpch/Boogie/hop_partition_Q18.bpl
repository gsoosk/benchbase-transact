...
procedure verify_hop_partitions_Q18(custkey: int, orderkey: int, linenumber: int)
{
  var s30_#tmp86 : Row (Table (Customer));
  var s30_custkey : int;
  var s30_c : Row (Table (Customer));
  var s30_#tmp87 : unit;
  var s31_#tmp88 : Row (Table (Orders));
  var s31_orderkey : int;
  var s31_o : Row (Table (Orders));
  var s31_#tmp89 : unit;
  var s31_l#l_quantity : real;
  var s31_linenumber : int;
  var s31_qty : real;
  var s31_#tmp91 : unit;

  // Hop partition verification for function 'Q18'
  s30_block30:
    s30_#tmp86 := construct_Row_Customer(Customer_c_custkey[s30_custkey], Customer_c_name[s30_custkey], Customer_c_address[s30_custkey], Customer_c_nationkey[s30_custkey], Customer_c_phone[s30_custkey], Customer_c_acctbal[s30_custkey], Customer_c_mktsegment[s30_custkey], Customer_c_comment[s30_custkey]);
    s30_c := s30_#tmp86;
    s30_#tmp87 := to_unit(s30_c);
    goto s30_hop_exit;
  s30_hop_exit:
  s31_block31:
    s31_#tmp88 := construct_Row_Orders(Orders_o_orderkey[s31_orderkey], Orders_o_custkey[s31_orderkey], Orders_o_orderstatus[s31_orderkey], Orders_o_totalprice[s31_orderkey], Orders_o_orderdate[s31_orderkey], Orders_o_orderpriority[s31_orderkey], Orders_o_clerk[s31_orderkey], Orders_o_shippriority[s31_orderkey], Orders_o_comment[s31_orderkey]);
    s31_o := s31_#tmp88;
    s31_#tmp89 := to_unit(s31_o);
    s31_l#l_quantity := Lineitem_l_quantity[s31_orderkey][s31_linenumber];
  // Partition check hop 31 func 'f' tables 'Orders'=>'Lineitem' keys [k0=orderkey] first_span Span { start: 11043, end: 11071, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 11124, end: 11180, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 30) (hop_id . 31) (table_id . 7) (span ((start . 11124) (end . 11180) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s31_orderkey == s31_orderkey);
    s31_qty := s31_l#l_quantity;
    s31_#tmp91 := to_unit(s31_qty);
    goto s31_epilogue;
  s31_hop_exit:
  s30_epilogue:
  s31_epilogue:
