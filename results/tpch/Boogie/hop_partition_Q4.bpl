...
procedure verify_hop_partitions_Q4(orderkey: int, linenumber: int)
{
  var s8_o#o_orderpriority : String;
  var s8_orderkey : int;
  var s8_#tmp23 : Row (Table (Lineitem));
  var s8_linenumber : int;
  var s8_l : Row (Table (Lineitem));
  var s8_#tmp24 : unit;
  var s8_priority : String;
  var s8_#tmp25 : unit;

  // Hop partition verification for function 'Q4'
  s8_block8:
    s8_o#o_orderpriority := Orders_o_orderpriority[s8_orderkey];
    s8_#tmp23 := construct_Row_Lineitem(Lineitem_l_orderkey[s8_orderkey][s8_linenumber], Lineitem_l_linenumber[s8_orderkey][s8_linenumber], Lineitem_l_partkey[s8_orderkey][s8_linenumber], Lineitem_l_suppkey[s8_orderkey][s8_linenumber], Lineitem_l_quantity[s8_orderkey][s8_linenumber], Lineitem_l_extendedprice[s8_orderkey][s8_linenumber], Lineitem_l_discount[s8_orderkey][s8_linenumber], Lineitem_l_tax[s8_orderkey][s8_linenumber], Lineitem_l_returnflag[s8_orderkey][s8_linenumber], Lineitem_l_linestatus[s8_orderkey][s8_linenumber], Lineitem_l_shipdate[s8_orderkey][s8_linenumber], Lineitem_l_commitdate[s8_orderkey][s8_linenumber], Lineitem_l_receiptdate[s8_orderkey][s8_linenumber], Lineitem_l_shipinstruct[s8_orderkey][s8_linenumber], Lineitem_l_shipmode[s8_orderkey][s8_linenumber], Lineitem_l_comment[s8_orderkey][s8_linenumber]);
  // Partition check hop 8 func 'f' tables 'Orders'=>'Lineitem' keys [k0=orderkey] first_span Span { start: 5608, end: 5636, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 5669, end: 5725, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 20) (hop_id . 8) (table_id . 7) (span ((start . 5669) (end . 5725) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s8_orderkey == s8_orderkey);
    s8_l := s8_#tmp23;
    s8_#tmp24 := to_unit(s8_l);
    s8_priority := s8_o#o_orderpriority;
    s8_#tmp25 := to_unit(s8_priority);
    goto s8_epilogue;
  s8_hop_exit:
  s8_epilogue:
}
