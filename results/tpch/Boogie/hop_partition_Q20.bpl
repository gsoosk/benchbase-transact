...
procedure verify_hop_partitions_Q20(partkey: int, suppkey: int, nationkey: int, orderkey: int, linenumber: int)
{
  var s34_#tmp98 : Row (Table (Part));
  var s34_partkey : int;
  var s34_p : Row (Table (Part));
  var s34_#tmp99 : unit;
  var s34_#tmp100 : Row (Table (Partsupp));
  var s34_suppkey : int;
  var s34_ps : Row (Table (Partsupp));
  var s34_#tmp101 : unit;
  var s35_#tmp102 : Row (Table (Supplier));
  var s35_suppkey : int;
  var s35_s : Row (Table (Supplier));
  var s35_#tmp103 : unit;
  var s36_#tmp104 : Row (Table (Nation));
  var s36_nationkey : int;
  var s36_n : Row (Table (Nation));
  var s36_#tmp105 : unit;
  var s37_l#l_quantity : real;
  var s37_orderkey : int;
  var s37_linenumber : int;
  var s37_qty : real;
  var s37_#tmp107 : unit;

  // Hop partition verification for function 'Q20'
  s34_block34:
    s34_#tmp98 := construct_Row_Part(Part_p_partkey[s34_partkey], Part_p_name[s34_partkey], Part_p_mfgr[s34_partkey], Part_p_brand[s34_partkey], Part_p_type[s34_partkey], Part_p_size[s34_partkey], Part_p_container[s34_partkey], Part_p_retailprice[s34_partkey], Part_p_comment[s34_partkey]);
    s34_p := s34_#tmp98;
    s34_#tmp99 := to_unit(s34_p);
    s34_#tmp100 := construct_Row_Partsupp(Partsupp_ps_partkey[s34_partkey][s34_suppkey], Partsupp_ps_suppkey[s34_partkey][s34_suppkey], Partsupp_ps_availqty[s34_partkey][s34_suppkey], Partsupp_ps_supplycost[s34_partkey][s34_suppkey], Partsupp_ps_comment[s34_partkey][s34_suppkey]);
  // Partition check hop 34 func 'f' tables 'Part'=>'Partsupp' keys [k0=partkey] first_span Span { start: 11968, end: 11992, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 12046, end: 12096, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 32) (hop_id . 34) (table_id . 4) (span ((start . 12046) (end . 12096) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s34_partkey == s34_partkey);
    s34_ps := s34_#tmp100;
    s34_#tmp101 := to_unit(s34_ps);
    goto s34_hop_exit;
  s34_hop_exit:
  s35_block35:
    s35_#tmp102 := construct_Row_Supplier(Supplier_s_suppkey[s35_suppkey], Supplier_s_name[s35_suppkey], Supplier_s_address[s35_suppkey], Supplier_s_nationkey[s35_suppkey], Supplier_s_phone[s35_suppkey], Supplier_s_acctbal[s35_suppkey], Supplier_s_comment[s35_suppkey]);
    s35_s := s35_#tmp102;
    s35_#tmp103 := to_unit(s35_s);
    goto s35_hop_exit;
  s35_hop_exit:
  s36_block36:
    s36_#tmp104 := construct_Row_Nation(Nation_n_nationkey[s36_nationkey], Nation_n_name[s36_nationkey], Nation_n_regionkey[s36_nationkey], Nation_n_comment[s36_nationkey]);
    s36_n := s36_#tmp104;
    s36_#tmp105 := to_unit(s36_n);
    goto s36_hop_exit;
  s36_hop_exit:
  s37_block37:
    s37_l#l_quantity := Lineitem_l_quantity[s37_orderkey][s37_linenumber];
    s37_qty := s37_l#l_quantity;
    s37_#tmp107 := to_unit(s37_qty);
    goto s37_epilogue;
  s37_hop_exit:
  s34_epilogue:
  s35_epilogue:
