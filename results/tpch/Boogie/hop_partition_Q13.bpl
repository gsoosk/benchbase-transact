...
procedure verify_hop_partitions_Q13(custkey: int, orderkey: int)
{
  var s20_#tmp62 : Row (Table (Customer));
  var s20_custkey : int;
  var s20_c : Row (Table (Customer));
  var s20_#tmp63 : unit;
  var s21_#tmp64 : Row (Table (Orders));
  var s21_orderkey : int;
  var s21_o : Row (Table (Orders));
  var s21_#tmp65 : unit;

  // Hop partition verification for function 'Q13'
  s20_block20:
    s20_#tmp62 := construct_Row_Customer(Customer_c_custkey[s20_custkey], Customer_c_name[s20_custkey], Customer_c_address[s20_custkey], Customer_c_nationkey[s20_custkey], Customer_c_phone[s20_custkey], Customer_c_acctbal[s20_custkey], Customer_c_mktsegment[s20_custkey], Customer_c_comment[s20_custkey]);
    s20_c := s20_#tmp62;
    s20_#tmp63 := to_unit(s20_c);
    goto s20_hop_exit;
  s20_hop_exit:
  s21_block21:
    s21_#tmp64 := construct_Row_Orders(Orders_o_orderkey[s21_orderkey], Orders_o_custkey[s21_orderkey], Orders_o_orderstatus[s21_orderkey], Orders_o_totalprice[s21_orderkey], Orders_o_orderdate[s21_orderkey], Orders_o_orderpriority[s21_orderkey], Orders_o_clerk[s21_orderkey], Orders_o_shippriority[s21_orderkey], Orders_o_comment[s21_orderkey]);
    s21_o := s21_#tmp64;
    s21_#tmp65 := to_unit(s21_o);
    goto s21_epilogue;
  s21_hop_exit:
  s20_epilogue:
  s21_epilogue:
