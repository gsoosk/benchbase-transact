...
procedure verify_hop_partitions_Q1(orderkey: int, linenumber: int)
{
  var s2_l#l_discount : real;
  var s2_orderkey : int;
  var s2_linenumber : int;
  var s2_l#l_extendedprice : real;
  var s2_#tmp1 : real;
  var s2_#tmp2 : real;
  var s2_revenue : real;
  var s2_#tmp3 : unit;

  // Hop partition verification for function 'Q1'
  s2_block2:
    s2_l#l_discount := Lineitem_l_discount[s2_orderkey][s2_linenumber];
    s2_l#l_extendedprice := Lineitem_l_extendedprice[s2_orderkey][s2_linenumber];
  // Partition check hop 2 func 'f' tables 'Lineitem'=>'Lineitem' keys [k0=orderkey] first_span Span { start: 3774, end: 3830, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" } current_span Span { start: 3774, end: 3830, filename: "/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact" }
    assert {:msg "(PartitionFunctionInconsistency (partition_function_id . 15) (function_id . 17) (hop_id . 2) (table_id . 7) (span ((start . 3774) (end . 3830) (filename . \"/Users/farzad/Desktop/Research/benchbase-transact/tpch.transact\"))))"} (s2_orderkey == s2_orderkey);
    s2_#tmp1 := 1.0000000 - s2_l#l_discount;
    s2_#tmp2 := s2_l#l_extendedprice * s2_#tmp1;
    s2_revenue := s2_#tmp2;
    s2_#tmp3 := to_unit(s2_revenue);
    goto s2_epilogue;
  s2_hop_exit:
  s2_epilogue:
}
